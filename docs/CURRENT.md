...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-PERF-02 Minimap 물리 오염 제거 후 60봇 Forward+ 성능 재기준화 |
| 최신 검증 개발 단위 | N2-NAV-03 Minimap/TestMap UID 오염 제거와 물리 UI 분리 gate |
...
| P4 | 맵/경로 체감 점검 | 숨은 TestMap 충돌체 제거와 올바른 월드 기준선 확정. 성능·수동 기준 재확인 전 맵 장애물 추가 금지 |
...
| POI 교전 유출 | 숨은 TestMap 물리가 있던 위치 기준선도 폐기. 올바른 월드 60/99봇 open 피해 63.0/69.6%, loot-hub 14.4/13.0% | 최종 맵 해법으로 해석하지 않고 성능·수동 체감 뒤 병목 후보를 한 개만 검증 |
...
| 60봇 프레임 버벅임 | N2-PERF-01 p95 12.6-13.5ms로 수동 통과했지만 올바른 월드에서 AI 평균이 오염 기준 260.7→317.9µs | 같은 Forward+ 조건을 다시 측정해 프레임 기준선을 확정한 뒤에만 최적화 여부 결정 |
