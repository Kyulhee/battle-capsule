...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-PLAY-01 확장 Night 수동 첫 1분 검증 |
| 최신 검증 개발 단위 | N2-AI-06 플레이어 대상 전투만 국소 캐시·분리 |
| 최신 검증 게임플레이 단위 | Arena 강제 1.1m 쌍 0.2초 분리·표적 유지 100%. Night 60/99봇에서 정체·이탈은 대조군 범위, AI 평균 비용 증가는 최대 6.1% |
...
| P2 | 수동 플레이 기준 승격 | 현재: 확장 후보 `xlarge_60` 첫 1분의 조우·가림·병목·대인 압박 판정 |
