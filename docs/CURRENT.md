...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-PLAY-02 `xlarge_60` 성능 개선 수동 재확인 |
| 최신 검증 개발 단위 | N2-PERF-01 60봇 AI 프레임 분산·검색 캐시·텔레메트리 배치 |
| 최신 검증 게임플레이 단위 | 수동: 조우·간격·공격·추적·사망 압박 적절. 자동: 프레임 p95 13.5ms, 33ms 초과 0.04%, 99봇 AI 평균 476→353µs |
...
| P2 | 수동 플레이 기준 승격 | 현재: AI 전투 체감은 통과, N2-PERF-01 뒤 버벅임 재확인 필요 |
...
| 60봇 프레임 버벅임 | 변경 전 프레임 p95 45.5ms, 33ms 초과 12.8%. N2-PERF-01 자동 반복은 p95 12.6-13.5ms, 33ms 초과 0.04-0.11% | `xlarge_60` 실제 전투에서 체감 재확인. 99봇 확장 whitebox disengage gate 실패와 성능 회귀를 혼동하지 않음 |
