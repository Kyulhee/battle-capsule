...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-AI-06 Night 규모 분산 회귀 |
| 최신 검증 개발 단위 | N2-AI-05 perception 국소 캐시 + 전투원 분리 조향 |
| 최신 검증 게임플레이 단위 | Arena 자연 기준선 접촉 3/5회·최대 7.65%에서 수정 후 0/5회. 강제 1.1m 쌍은 5/5회 0.15-0.25초 안에 1.5m 밖으로 분리 |
...
| P1 | AI 교전 결정 재설계 | Arena 목표 기억·국소 분산 완료. 다음: Night 60/99봇에서 AI update·stuck·이탈 회귀 확인 |
...
| squad 근접 겹침 | 기준선 5-run 중 3회 1.5m 미만, 최소 0.97m. 국소 분산 뒤 5-run 모두 접촉률 0%, 최소 1.89m | ATTACK/비loot CHASE와 현재 표적 제외 계약 유지, Night 규모 비용·pathing 재검증 |
