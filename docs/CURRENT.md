...
| 바로 다음 단위 | E-076: 빈 탄약 RECOVER/IDLE의 실제 검색 호출에서 캐시·거리·감지·호환/장비 필터 탈락을 구분하는 선택적 진단. 기존 탐색을 재호출하지 않고 초기ID/기본 비활성 비용을 검증한다. E-075 phase 시점만으로 검색 실패 원인을 판정하지 않는다. E-071 opt-in 유지, 기본 적용·드랍/시야 변경·새 EXE 없음 |
| 최신 검증 개발 단위 | E-075 stage2 시작+1초 관측·분석기/21종 오류 입력·최종 초기ID fixture PASS. 5배4창 중 지연0.396초 실패를 보존하고 전용 창을 실시간으로 분리했다. 실시간seed41001 대조/후보는 지연0.0039/0.0065초·재고208/201·생존/빈 탄약8/3과9/1, phase/원시ID exact PASS. 단일 진단 쌍이지5-run/수동 승격 아님. 전체 회귀 첫 wall_traffic 실패 보존·독립/전체 재실행 PASS. `builds/verification/E075_loot_phase/RESULTS.md` 참조, 수동 hash불변 |
