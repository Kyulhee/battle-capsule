...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-NAV-01 고형 `rock_cluster` ramp·nav 정체 소형 재현 |
| 최신 검증 개발 단위 | N2-MAP-07 open 피해 셀·인접 POI 계측과 Bot 전략 문맥 통합 |
...
| P4 | 맵/경로 체감 점검 | 2차 진단: open 피해 상위 셀은 기존 바위·나무 주변에 분산. 고형 바위 정체부터 Arena에서 격리 |
...
| 경로 이탈 | 기존 바위 `(-6,-48)`, `(8,46)` 셀이 60/99봇 open 피해 약 10-11%와 정체 약 15-23%를 함께 차지 | 맵 위치를 다시 옮기기 전에 Arena에서 high rock ramp·nav 구조를 재현하고 생성기 문제와 배치 문제를 분리 |
