...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-MAP-07 open 전투 누출 셀·인접 POI 계측 |
| 최신 검증 개발 단위 | N2-MAP-06 `Central Meadow` 바위 엄폐 후보 폐기와 검증 맵 경로 고정 |
...
| P4 | 맵/경로 체감 점검 | 2차 진단: 중앙 바위 엄폐는 open 피해를 줄였지만 새 정체 셀을 반복 생성. open 전투 셀부터 계측 |
...
| POI 교전 유출 | 중앙 바위 후보는 open 피해 71.2→69.5%, loot-hub 피해 9.0→15.8%로 이동시켰지만 새 바위 셀 정체가 0→15회 발생 | 장애물을 다시 옮기지 않고 open 피해 셀·인접 POI를 먼저 계측해 실제 유출 경계를 특정 |
