...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-MAP-05 Night 맵 규모·지형 밀도 재설계 |
| 최신 검증 개발 단위 | N2-AI-01 플레이어 표적 위협 판정 분리 |
| 최신 검증 게임플레이 단위 | 수동 플레이: 정지 시 접촉 부족, 4봇이 플레이어를 견제한 뒤 이탈, 플레이어 과소 사망 |
...
| P1 | 매치 길이 안정화 | 대기: 180m/99봇 과밀 구조를 새 맵 후보에서 재설계한 뒤 재개 |
| P2 | 수동 플레이 기준 승격 | 1차 완료: 자동 PASS였던 route 표현을 실제 조작에서 폐기 |
| P3 | 야간 가독성 개선 | 완료: T3 1차, Night 전용 주변광/달빛과 deterministic 대비 gate 적용 |
| P4 | 맵/경로 체감 점검 | 다음: 180m·장애물 점유 4.2% 기준을 폐기하고 자연스러운 병목 후보를 설계 |
...
| 플레이어 과소 사망 | 수동 플레이에서 4봇이 견제 후 이탈했고 위협을 찾아 이동하지 않으면 접촉이 적음 | 대인 target 유지, 사거리·시야, DISENGAGE 전이와 추적 중단 이유를 순서대로 audit |
...
| 구조적 교전 밀도 | 180m 맵은 30초 직선 횡단, 28개 장애물 추정 점유 4.2% | 좌표만 확대하지 말고 pickup·시야 차단·병목·이동 시간과 물리 점유를 함께 설계 |
| route 오해 | AI가 소비하지 않는 route 표시가 전략 경로처럼 보여 이질감과 오해를 만듦 | route는 진단 분류로만 유지하고 실제 이동 계약 전에는 플레이어에게 표시하지 않음 |
| 전체 지도 혼잡 | 45도 방향은 정렬됐지만 중앙 POI 이름이 겹침 | 맵 구조 재설계 때 label 우선순위와 확대/선택 표현을 함께 정리 |
