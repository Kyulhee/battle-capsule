...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-PLAY-03 중앙·남쪽 거점 엄폐와 픽업 접근 수동 판정 |
| 최신 검증 개발 단위 | N2-MAP-10 공간 감사와 자연 엄폐 후보 검증 |
...
| 테스트 표면 오판 | 96m 고정 스폰 실험 결과를 실제 Night 맵 페이싱으로 확대 해석 | Arena는 재현·회귀용으로만 사용. wall 도달 허용은 5.0m, 제품 hotspot은 실제 260m 맵에서 별도 검증 |
