...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-PLAY-05 저빈도 POI 전략 이동과 자세·전투 오디오 수동 재판정 |
| 최신 검증 개발 단위 | N2-AI-08 IDLE 전략 목적지, N2-ASSET-03 앉기·권총·칼 오디오 조정 |
| 최신 검증 게임플레이 단위 | N2-PLAY-04: 로컬 미니맵·AI 전투 개선 채택, 맵 전체 압력과 오디오 세부 반복 필요 |
...
| P4 | 맵/경로 체감 점검 | IDLE 봇이 3초 뒤 아키타입별 POI를 장기 목적지로 삼는 후보 적용. 실제 압력은 N2-PLAY-05 판정 |
| P5 | 자산 승격 | 앉기 발걸음 -10dB/청취 45%, 권총 -8.5dB, CC0 칼 Foley 후보 적용. 수동 재판정 대기 |
...
| 맵 전체 압력 부족 | 로컬 지도와 AI 전투는 개선됐지만 실제 플레이에서 거점으로 끌리거나 지역 사이를 이동하는 강제성이 느껴지지 않음 | 근거리 pickup 목표 뒤 정지하던 IDLE에 저빈도 POI 목적지를 연결. N2-PLAY-05에서 지역 간 이동과 과소모를 함께 판정 |
...
| 60봇 프레임 버벅임 | Main과 Player가 미니맵을 중복 생성하고 과거에는 정적 지형도 매 프레임 그림 | 단일 미니맵·정적 캐시·전략 이동 후보에서 p95 12.9ms, p99 14.8ms |
| 전투·자세 오디오 균형 | 새 권총이 다른 총보다 크고 칼 procedural noise가 부자연스러우며 앉기도 같은 발걸음 음량 | 권총 -8.5dB, CC0 swish Foley, 앉기 -10dB/AI 청취 45% 후보. 재청취 전 승격 금지 |
