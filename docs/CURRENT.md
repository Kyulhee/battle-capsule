...
> 최종 업데이트: 2026-08-15. 작업 시작 전 이 문서를 먼저 읽는다.
...
| 항목 | 값 |
|---|---|
| 현재 단위 | behavior-neutral continuity schema v2와 5-run 기준선을 완료했다. exact aggregate는 59초 이내 생존 상태 unique release 414·1초 내 같은 대상 재획득 132(31.9%)·opening DISENGAGE exit 1284를 완전 집계하고 raw는 run마다 release episode/exit 각각 최대 128개의 결정적 bounded sample로 제한한다 |
| 바로 다음 단위 | `check_scale_telemetry`는 통과했지만 기록된 `tactics.disengage_entries` 보조 watch가 2/5에서 0.70을 넘었다. 새 gameplay 수치를 더 섞기 전에 behavior-neutral opening 생존 상태 노출 분모로 위치·상태 집중을 좁히고, 단일 후보가 1-run 방향 gate를 통과할 때만 5-run·packaged 수동 3판으로 진행 |
...
| 최신 검증 게임플레이 단위 | behavior-neutral v2 5-run은 평균 668.8초(531.5-805.5), alive 중앙값 `54/34/24/23/21/15`, T50/T10 `31.5/306.7초`, first upgrade 3.5초였다. `check_scale_telemetry`만 PASS이고 생존 watch band는 FAIL, 기록된 DISENGAGE entry 보조 watch도 2/5 초과라 gameplay PASS가 아님 |
...
| 우선순위 | 작업 | 종료 조건 |
|---|---|---|
| P0 | `N2-PLAY-11` M1 실패 수정·재판정 | opening 생존 상태의 위치·시간 노출 분모로 반복 이탈/근거리 집중을 먼저 분리하고, 그 근거를 겨냥한 새 단일 후보가 1-run 방향 gate를 통과할 때만 5-run으로 확대해 생존·continuity 통과 뒤 packaged 수동 3판을 승인 |
...
| 리스크 | 신호 | 대응 |
|---|---|---|
| M1 생존·continuity 실패 | v2 5-run alive@30/60/120 `54/34/23`, T10 306.7초. release 414 중 1초 내 재획득 132(31.9%)이고 기록된 `tactics.disengage_entries/(spawned×duration_min)` 보조 watch가 run2 0.782·run4 0.864로 0.70을 초과 | ceasefire·HP buffer·1초 counteraction grace는 완전 revert하고 5-run 금지. 위치·상태·노출 원인을 좁힌 새 후보가 생존 증가와 재획득·정체·이탈 비회귀를 같은 1-run에서 증명 |
