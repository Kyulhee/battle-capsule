...
| 항목 | 값 |
|---|---|
| 현재 단위 | E-066 `N2-ITEM-VIS-01` 구현·패키지 검증 완료, 수동 대기. clean 소스 `e16b089`; Windows `builds/playtest/E-066_e16b089/BattleCapsule_E066_e16b089.exe`, 메뉴 `v2.1.0-demo-dev \| E-066`. macOS ZIP·checksum은 같은 폴더, 실기기 Mac 미검증 |
| 바로 다음 단위 | `N2-LOOT-FLOW-01` E-067: 무효 탄약 수집/같은 계열 업그레이드 예비탄 손실을 먼저 수정하고 패키지로 확인한다. 이후 Survey Camp↔Central Meadow 유한 보급의 탄종 구성 후보를 별도로 검증한다. 120/260초 재고·이동·생존 계측은 남아 있으며 후방 급회전·폭격·건물은 섞지 않음 |
| 최신 검증 개발 단위 | E-066 전체 `unit_smoke` PASS. tier fixture는 획득 경로/색 독립성, 등급 비교, 가림, 슬롯 상태/업그레이드 규칙을 확인했다. Forward+ 실제 Night 60봇 표면의 정지 fixture 720p/1080p×6상태 12캡처에서 라벨·HUD 경계/중앙 정렬 PASS. 새 페이싱 5-run이나 사람 완주 근거는 아님 |
