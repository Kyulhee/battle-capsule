...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-AI-05 `squad_4` 국소 전투원 분산 |
| 최신 검증 개발 단위 | N2-AI-04 플레이어 표적 기억 5초 + 실제 squad runtime gate |
| 최신 검증 게임플레이 단위 | Arena squad 격리 구간: 유지율 100%, 동시 표적 4명, 공격 참여 4명. Night 수동 재검증 대기 |
...
| P1 | AI 교전 결정 재설계 | 목표 기억 완료. 다음: 전역 순회 없이 `squad_4` 최소 간격 1m대 겹침 완화 |
...
| squad 근접 겹침 | 자연/강제 교전 probe에서 봇 간 최소 거리가 약 1.0m까지 축소 | 기존 perception 근접 목록만 쓰는 국소 분산 후보를 Arena에서 먼저 검증 |
