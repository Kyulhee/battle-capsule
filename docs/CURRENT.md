...
| 바로 다음 단위 | E-074: 계측OFF에도 진단 객체를 생성하던 probe를 지연 로딩으로 수정하고, 부모 `9f23dde`와 계측OFF/ON의 초기 스냅샷을 ID까지 exact 검증했다. 회귀 검증 뒤 새 출력 폴더에서 비계측5+5를 다시 수행한다. E-073은 초기 ID 기반 선택의 교란으로 중단했으며 양군4-run씩 보존·승격 제외. 기본 적용·드랍/시야 변경·새 EXE는 없음 |
| 최신 검증 개발 단위 | E-072 계측 대조/후보 각2-run·합계128,879표본에서50ms 초과0, max대조35,381/18,136us·후보26,467/24,522us. 양군 진단 gate·계측 합계/보존/배선·전체 unit_smoke PASS. 실시간 후보 창261표본·지연0.0128초·재무장 관측16회, 5초 초과 동일 목표 접근 뒤 재무장을 확인했다. 생존/보급 개선 승격은 아님. 수동 hash불변 |
