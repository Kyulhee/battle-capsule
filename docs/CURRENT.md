...
| 바로 다음 단위 | E-071 후보: 정상 접근 중에도 5초가 지나면 포기하는 아이템 추적을 소형 재현으로 격리하고, 진행 기반 정체 판정과 유한 총 추적 시간의 계약을 먼저 검증한다. 탄약 고갈의 주원인으로 단정하지 않으며 회복 탐색/순찰과 필요 기반 목적지 진단을 유지한다. 전역 드랍 증가·시야 확장·선점 해제·새 건물을 섞지 않는다. 제품 맵/EXE는 E-067 유지, 새 수동 요청 없음 |
| 최신 검증 개발 단위 | E-070 실시간 260초 창은 261표본·최대 지연 0.012초로 품질 PASS. 빈 탄약 1,557표본 중 RECOVER/IDLE/CHASE `831/490/56`, 재무장 관측 6회. 약 29→15m 정상 접근 뒤 같은 탄약 재추적 사례를 확인했다. alive@120/260 `17/8`은 단발 진단이며 전체 매치/생존 승격이 아니다. 5배 가속 2-run은 구조 PASS지만 초반 지연으로 정밀 진행 분석 제외. E-068 기본 미적용 유지 |
...
| 브랜치 메모 | 사용자 승인 범위인 기존 진단 `04bdb46`과 README 갱신 `1ddfd9c`까지 원격 master 반영을 확인했다. 이후 새 작업의 푸시는 대상 commit을 명시한 승인을 다시 받은 뒤에만 한다 |
