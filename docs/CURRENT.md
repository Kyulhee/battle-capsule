...
> 최종 업데이트: 2026-09-11. 작업 시작 전 이 문서를 먼저 읽는다.
...
| 바로 다음 단위 | E-073: 부모 `9f23dde` 별도 worktree 대조군과 E-071 후보의 비계측5+5를 같은 맵/preset·입력41000-41004로 순차 비교한다. E-072 계측 실행은 여기에 세지 않는다. 과거1.19초 실패는 원인 미확정으로 보존하며 새 실행으로 지우지 않는다. 기본 적용·드랍/시야 변경·새 EXE는 없음 |
