...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-MAP-06 `Central Meadow` pickup 수렴·개방지 유출 단일 화이트박스 |
| 최신 검증 개발 단위 | N2-AI-07 spawn·loot 목적지 POI 수렴 계측 |
...
| P4 | 맵/경로 체감 점검 | 2차 준비: pickup 목적지는 POI/route를 소비하지만 피해 약 70%가 open. `Central Meadow`부터 단일 검증 |
...
| POI 교전 유출 | 60/99봇 spawn은 POI 내부 약 26%지만 IDLE loot 목표는 66-67%가 내부, 약 90%가 route 위. 피해는 open 70-71% | AI route 강제 대신 pickup 수렴이 가장 큰 `Central Meadow`의 입구·엄폐·개방 경계를 단일 변수로 검증 |
