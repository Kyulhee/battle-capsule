...
| 항목 | 값 |
|---|---|
| 현재 단위 | N2-PLAY-03 중앙·남쪽 거점 동선과 핵심 전투·발걸음 오디오 수동 판정 |
| 최신 검증 개발 단위 | N2-ASSET-01 핵심 오디오 7종 런타임 승격과 스트림 캐시 |
...
| P4 | 맵/경로 체감 점검 | 자동 검증 완료: 중앙·남쪽 거점 자연 엄폐 후보. `xlarge_60` 수동 판정 대기 |
| P5 | 자산 승격 | 1차 완료: 무기 4종·지면 발걸음 3종 런타임 연결. 수동 청취 뒤 landmark 시각·충돌 계약 감사 |
...
| 60봇 프레임 버벅임 | 실제 Minimap이 정적 지형 64개를 매 프레임 그려 p95 36.6-39.1ms, 33ms 초과 9.2-13.1%가 재현됨 | 정적 지도 1회 캐시 뒤 p95 15.4-15.8ms, 33ms 초과 0.14-0.27%. 정적/동적 레이어 분리 gate 유지 |
| 오디오 런타임 비용 | 실제 WAV를 발사·이동마다 다시 찾으면 프로세스 비용이 흔들릴 수 있음 | 성공 스트림을 ID별로 캐시. 60봇 2회 p95 15.5-16.5ms, p99 20.7-26.6ms 유지 |
