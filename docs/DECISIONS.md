...
> 최종 업데이트: 2026-09-14. 안정된 결정을 짧게 유지한다. 상세 경위는 devlog와 experiments에 둔다.

## 재검토 전까지 유지

E-073/E-074 보완: 비활성 진단은 Resource/RefCounted/Node도 추가 생성하지 않는다. 현재 AI는 instance ID를 엄폐·조향·분산 선택에 쓰므로, 비교 전 초기 snapshot은 ID를 제외하지 않고 검증한다. 계측ON의 이후 실행이 행동 결정적이라고 가정하지 않으며 D-015는 유지한다.
...
| D-004 | `night_br_m1_60`의 자동 페이싱 gate는 duration 평균 600-900초·개별 480-960초, 경기별 최초 non-pistol 기록의 평균 2-30초다(최소 5-run) | duration은 평균만 보면 이상치를 놓친다. 초기 총기 부족을 해결한 N2-EQUIP-01 5-run이 평균 746.7초·first 4.5초로 세 조건을 함께 통과. 개별 획득 2초 하한이나 스폰 최소 접근 거리 계약은 아니다 | 수동 3판이 초기 즉사 또는 10-15분 흐름을 거부하거나 결정적 실행기가 도입됨 |
...
- 2026-09-06 계획 결정: 기존 티어 가독성을 먼저 통일하고, 이동 압력은 총/호환 탄약/장비 필요와 유한 거점 보급으로 검증한다. 이후 기존 건물 한 동의 문·내외 시야·창 사격을 별도 후보화한다. 폭격·붕괴·보라 특수 장비는 M1/R0 필수 요구로 올리지 않는다. 색상 상세·특수 효과 수치는 MASTERPLAN의 잠정안이며 지금 구현을 확정한 것이 아니다.

- E-065 소리 반응은 player-only cue snapshot 0.6초와 비전투 시선 우선권으로 고친다. 맵·청취 거리·반응 지연 수치·bot끼리의 cap/시선 규칙을 함께 바꾸지 않는다. 격리 반응 PASS는 실제 매치 압력이나 5-run 페이싱 PASS가 아니다.
