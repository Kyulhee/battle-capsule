...
> 최종 업데이트: 2026-07-18. 안정된 결정을 짧게 유지한다. 상세 경위는 devlog와 experiments에 둔다.
...
| D-023 | 봇 분리 조향은 LOS가 있는 3m 국소 캐시를 쓰고 ATTACK/비loot CHASE에서 현재 표적을 제외한다 | 전역 후보 조회 없이 squad 1.5m 미만 접촉을 5-run 3회에서 0회로 줄이고 loot·직접 교전 계약을 보존 | Night 규모에서 AI update, stuck, 이탈이 유의하게 악화 |
