...
- Follow-up material correction keeps catalog bush visuals and the interior tint unshaded/non-shadow-casting, but restores depth testing and backface culling to avoid transparent GLB overdraw artifacts.
- Catalog bush visuals now cut away from 0.72 alpha to 0.24 alpha while the player is inside the same bush; the old cylinder feedback mesh is converted into a thin low-alpha floor tint instead of a tall dome.
...
- `python tools\simulate_matches.py 1 normal` 통과: duration=49.2s, stage=2, recover=24, disengage=22.
