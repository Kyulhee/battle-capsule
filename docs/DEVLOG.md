...
## N2-MAP-06 중앙 초원 엄폐 후보 폐기

- 후보: `Central Meadow` 가장자리에 고엄폐 2개를 놓은 뒤 서쪽 회귀를 제거하고 동쪽 1개, 좁은 수평 면적까지 단계적으로 비교했다.
- 결과: 동일 시드 60봇에서 최종 후보는 open 피해 71.2→69.5%, loot-hub 피해 9.0→15.8%, normalized stuck 0.20→0.16이었지만 새 바위 셀 정체가 0→15회로 5경기 모두 반복됐다.
- 결정: 맵 변경과 전용 테스트를 제거하고 99봇 확대를 중단했다. 다음은 open 피해 셀과 인접 POI를 계측해 실제 유출 경계를 먼저 특정한다.
- 검증 보강: 잘못된 기본 맵 실행 1회는 폐기했다. `pacing_candidate`는 이제 `--map-spec-path`가 없으면 시작하지 않는다.
