...
- 아티팩트 선택 화면: `"없이 시작"` → `"선택하지 않기"`, 자막 간결화
- 난이도 설명: `"배틀로얄 첫 입문에 추천."` → `"입문용 난이도."`, `"표준 봇 성능 · 균형 잡힌 전투."` → `"표준 난이도."`, `"랜덤 모디파이어"` → `"랜덤 이벤트"`
- Red Trigger: `"비샷건"` → `"샷건 외"` (2회)
- Armor Sponge: `"힐→쉴드"` → `"힐→방어막"`, `"이동속도"` → `"이동 속도"`, 힐 설명 자연어화
- Silent Core: `"달리기 소리 탐지 완전 차단"` → `"달리기 소음 탐지 차단"`
- Zone Battery: `"힐 아이템 / 방어구 픽업 완전 봉인"` → `"힐·방어구 사용 불가"`
- 조작키 버그 수정: 아이템 줍기 `E` → `F` (컨트롤 가이드 미반영 수정)
- 아이템 명칭: `치료 아이템` / `소힐` / `메딕킷` → `붕대`(♥, +30 HP) / `구급상자`(◆, +60 HP)
- HUD 아이콘 설명: `"치료 아이템 보유 수"` / `"고급 치료제 보유 수"` → `"붕대 보유 수"` / `"구급상자 보유 수"`
- 무기 명칭 통일: `"AR"` / `"어설트 라이플"` → `"돌격소총"` (MissionTracker HUD)

**src/core/MissionTracker.gd**

- MEDIC RUN 설명·HUD: `MedKit(◆)` / `치료 아이템(◆)` → `구급상자(◆)`
- 무기 이름 사전: `"어설트 라이플"` → `"돌격소총"` (wnames, wnames2, gun_labels)

**README.md**

- 아이템 표: 치료 아이템 단일 행 → 붕대(♥, +30 점진) / 구급상자(◆, +60 즉시) 두 행으로 분리
- 조작법: `Q` 설명, 상태바 설명, 전략 팁 내 명칭 통일
- 보급 캡슐 수집 키 `E` → `F` 수정
