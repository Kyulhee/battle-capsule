...
| 현재 초점 | N2-MAP-17: Brush Camp 은폐, Survey Camp 도로 병목, 도로·숲·거점 점유 압력 |
...
- Brush Camp가 느린 숲 안의 은폐·회복 거점으로 읽히고 세 진입 방향이 막힘 없이 구분되는가?
- Survey Camp가 중앙 도로의 통과 병목으로 읽히되 우회 불가능한 벽이나 즉사 지점이 되지 않는가?
- 빠른 노출 도로와 느린 엄폐 숲 사이에서 이동 시간·위험·보상의 차이가 실제 선택을 만드는가?
- stage2가 회전 압박을 만들면서 stage3 도달을 막지 않는가?
- 죽음의 이유가 플레이어 관점에서 이해되는가?
- `night_br_m1_60`에서 이동과 다중 전투 중 프레임 끊김이 반복되는가?
- 중앙 초원에서 수목·수풀이 화면 가장자리에 읽히면서 중심 이동을 막지 않는가?
- 남쪽 보급 거점에서 왼쪽 수풀·수목과 오른쪽 바위 사이 접근 선택이 생기는가?
- 피스톨·AR·샷건·레일건이 소리로 구분되고 권총은 AR보다 작게 들리는가?
- 잔디·흙·돌 발걸음이 거리 판단을 돕고 앉았을 때 작아지되 끊기지는 않는가?
...
N2-MAP-17에서는 두 신규 거점이 프롭을 흩뿌린 장애물 집합이 아니라 파밍·엄폐·병목 역할이 다른 장소로 읽히는지 본다. Brush Camp 안팎의 숲 우회와 Survey Camp의 도로 통과를 한 판에서 비교하고, 거점 체류가 정체나 과도한 교전 집중으로 느껴지지 않는지 확인한다. Forward+ 자동 측정 3회 중 1회 p95 32.695ms 이상치가 있었으므로 반복 끊김 여부도 함께 기록한다. 중심·도로 T2/외곽 T1 장비 차이, 실드 충전과 방탄 조끼 구분, 첫 축소 120초, 구급상자·실드 6초 전투 잠금, 저체력 이동 저하도 회귀 확인한다.
