...
| 현재 초점 | N2-SURV-01: 첫 축소 압력, 교전 중 회복 제약, 저체력 이탈 속도 |
...
N2-SURV-01에서는 첫 축소가 120초에 시작해 이전의 긴 대기감을 줄이는지 본다. 총을 쏘거나 맞은 뒤 6초 동안 구급상자와 바닥 실드는 막히고 붕대만 사용되어야 한다. 구급상자는 1개만 들 수 있고 안전할 때 정확히 50HP를 회복한다. HP 50%/25% 이하의 이동 저하는 이탈 판단을 만들되 조작을 포기하게 해서는 안 된다.
