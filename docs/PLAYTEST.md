...
> 최종 업데이트: 2026-07-23. 텔레메트리가 말하지 못하는 체감과 화면 판단을 짧게 기록한다.
...
| 현재 초점 | N2-EQUIP-01: 지역별 총기 품질, 실드/방어구 구분, 초반 장비 선택 |
...
N2-EQUIP-01에서는 중심 거점·주요 길의 표준 총기와 외곽 수풀·회복 지역의 노후 총기가 실제 위험·보상 차이로 느껴지는지 본다. 실드 충전은 파란 즉시 자원이고, 방탄 조끼는 별도 장착물로 총기·근접 피해를 15% 줄이는 대신 이동을 4% 늦춰야 한다. 첫 비권총 접근이 빨라졌지만 첫 1분이 무작위 즉사로 바뀌어서는 안 된다. 함께 적용된 첫 축소 120초, 구급상자·실드 6초 전투 잠금, 저체력 이동 저하도 회귀 확인한다.
