...
> 마지막 업데이트: 2026-06-24 (verification profile runner 추가)
...
Use profiles to decide the smallest useful gate. Do not paste every command into active planning docs.
Run them through `tools/run_verify.py` when possible:
...
| Profile | Use When | Minimum Gate |
|---|---|---|
| `docs_only` | Markdown / tracker / planning changes only | `git diff --check` |
...
# generic playable pacing candidate profile:
...
# exact-threshold ammo need classification, and immediate-value loot priority.
