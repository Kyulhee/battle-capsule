...
| `src/ui/player/PlayerHudBuilder.gd` | Player top HUD node/style construction | Player가 CanvasLayer root를 넘기고 label/runtime values를 계속 소유 |
...
Entity (CharacterBody3D)
  ...
  공통 API: take_damage(), heal(), move_toward()
       ...
       │            top HUD construction: PlayerHudBuilder
...
| Player top HUD 레이아웃 | `src/ui/player/PlayerHudBuilder.gd` | Player가 label references를 보관하고 `_process()`/flash/kill feed에서 값만 갱신 |
| Player status/slot HUD 레이아웃 | `Player._ready()` health/shield/stat/slot 구성 블록 | CanvasLayer/Control 하위 — 순서가 z-order 결정 |
