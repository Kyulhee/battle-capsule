...
v1.11.22 기준 Bot debug marker 노드 생성은 `BotDebugLabelBuilder.gd`, marker text/color/catalog id mapping은 `BotMarkerFormatter.gd`가 소유한다. `Bot.gd`는 AI state, reveal/visibility checks, AssetCatalog 조회, visual skin application, and Telemetry hooks를 유지한다.
...
| Bot debug label nodes | `src/entities/bot/BotDebugLabelBuilder.gd` | Bot state/archetype text, visibility, reveal checks |
