...
v1.11.28 기준 pickup base colors, glow/light tuning, label LOD distances/scales/colors, visibility refresh interval, and icon plane size/height values are owned by `src/entities/pickup/PickupPresentation.gd`. Pickup icon ids, texture cache, AssetCatalog path lookup, ResourceLoader texture loading, and image-file fallback loading are owned by `src/entities/pickup/PickupIconResolver.gd`. `Pickup.gd` still owns runtime node creation, focus/LOS updates, cluster-label comparison, AssetCatalog scene lookup, icon decal placement/material setup, item collection side effects, Telemetry pickup logging, and debug logging.
...
| 압박 미션 풀 확장 | `src/systems/mission/MissionCatalog.gd` + `PressureEffectCatalog.gd` | 새 Effect 추가 시 catalog id/표시, `PressureEffectApplier.gd` 실행 케이스, 필요 시 Main state update 확인 |
...
| 보너스 미션 HUD 문구 | `MissionHudFormatter.gd` | `MissionTracker` bonus HUD context와 `MissionData.gd` condition ids 확인 |
| 보너스 미션 달성/early-fail 판정 | `MissionEvaluator.gd` | `MissionTracker` evaluation context, `MissionData.gd` condition ids, result/mission save flow 확인 |
...
| 보급 캡슐 로직 | `src/systems/loot/SupplyDropController.gd` + `src/systems/loot/LootSpawnDirector.gd` + Main supply state | Minimap이 `supply_telegraphed` / `supply_pos` 읽음 |
