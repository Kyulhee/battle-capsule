...
대부분의 코어 모듈은 RefCounted이며 씬트리 독립을 목표로 한다. 예외적으로 `HellEventController`는 RefCounted지만 런타임 시각 효과와 타이머를 위해 `Main.gd`가 넘긴 host/overlay parent를 사용한다. 이 경우에도 컨트롤러가 `Main.gd`를 직접 탐색하지 않고, `Main.gd`가 필요한 참조를 명시적으로 주입한다.
...
#### HellEventController (`src/core/HellEventController.gd`)
...
| 압박 미션 풀 확장 | `MissionTracker.get_hard_pool()` / `get_hell_pool()` | 새 Effect 추가 시 `PressureEffectApplier.gd`와 HUD effect formatter 동시 수정 |
...
| Hell 정전/포격 튜닝 | `HellEventController.gd` + `GameConfig.hell_value()` | Main modifier wiring, Player.gd SCARCITY reads, Telemetry hell event names |
