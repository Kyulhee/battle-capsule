...
v1.11.27 기준 pickup base colors, glow/light tuning, label LOD distances/scales/colors, visibility refresh interval, and icon plane size/height values are owned by `src/entities/pickup/PickupPresentation.gd`. `Pickup.gd` still owns runtime node creation, focus/LOS updates, cluster-label comparison, AssetCatalog icon loading, item collection side effects, Telemetry pickup logging, and debug logging.
