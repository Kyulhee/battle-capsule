...
| `src/systems/mission/MissionHudFormatter.gd` | 압박 미션 HUD title/progress/reward/penalty 문자열 포맷 | MissionTracker가 pressure state snapshot을 넘겨 사용 |
...
v1.11.6 기준 압박 미션 HUD 문자열 조립은 `src/systems/mission/MissionHudFormatter.gd`가 소유한다. `MissionTracker`는 pressure counters snapshot과 condition id mapping을 넘기며, 조건 평가와 public `get_pressure_hud_text()` API는 유지한다. 보너스 미션 HUD는 Telemetry/player HP 조회가 얽혀 있어 별도 slice로 남긴다.
...
MissionTracker ──uses──► MissionCatalog descriptor pools + filter_feasible()
               ──uses──► MissionHudFormatter pressure HUD strings
               (runtime state/evaluation owner — RefCounted)

MissionHudFormatter ──uses──► PressureEffectCatalog effect labels
...
| 새 PressureCondition | `MissionTracker` enum + `filter_feasible()` + `MissionCatalog` condition mapping | `_eval_single_condition()`와 progress HUD 케이스 추가 |
