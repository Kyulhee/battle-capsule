...
> 최종 업데이트: 2026-09-13. 같은 실패를 반복하지 않기 위한 짧은 장부다.

## 활성 판단

| ID | 질문 | 최신 증거 | 판단 |
|---|---|---|---|
| E-079 | 첫 수집0초가 미기록/이중 배속/초기 겹침인가? | 제품 코드 불변·실제 수집/장착/Telemetry1x/5x×5경우·전체 회귀 PASS. E078 초기1,080쌍 최소 수평4.16684m·2.5m 내0쌍. 통제된5배속 물리 handler burst는2m stub이동/실제 장착 뒤0초 기록,1배속은 미수집 | 미기록-1과 유효0 구분·이중 배속/초기 겹침 아님. 이동 후 시계 갱신 전 수집 가능성은 재현했으나 실제 E078 collector/frame 순서는 미확정.2초 하한/시계/스폰·실패 gate 유지, 다음 실제 시작 창의 성공 수집/프레임 연결 |
| E-078 | 빈 탄약 회복 순찰의 목적지를 지도상 보급 거점으로 바꾸면 유효한가? | 기본OFF 단일 후보·기존 policy 재사용·기본/fallback RNG·우선순위·6모드 ID/혼합 거부·전체 회귀 PASS. 1+1은663.9/678.6초·AI max25,943/25,020us, 빈 탄약120초10/23→6/25·260초6/16→8/19. 후보 생존 봇 선택24/21회 | opt-in 유지·5-run/기본 승격 보류. 양군 첫 upgrade0초로 전체 gate FAIL,260초 phase 불일치로 재고 비교 제외·단일 쌍/난수 흐름 변경 한계. 요약 스크립트 KeyError를 수정하며 완료 대조는 재실행하지 않음. 다음 초기 픽업/시계 재현·하한 유지 |
| E-077 | 실제 감지 탈락은 거리/FOV/LOS 중 무엇인가? | 실제 첫 반환·경계/장애물·결정/RNG/ray parity·초기5모드 ID·전체 회귀 PASS. 실시간54,195콜/fresh3,517, 감지 탈락23,567방문 중 거리84.932%/FOV13.447%/LOS1.621%, 예시26/누락54,169 exact. OFF546.6초·AI max41,112us·upgrade0.664초 | 진단 채택, 전체 sanity gate는 duration/upgrade 하한 FAIL로 보존. 아이템 방문 분포이지 검색별 원인·부족 시간·호환성/접근성 증거 아님. 다음 RECOVER 순찰 목적지 소형 재현/보급 지향 별도 후보; 전역 시야 확대·기본/EXE 승격 없음 |
| E-076 | 빈 탄약 IDLE/RECOVER 검색이 실제 어느 필터에서 실패하는가? | 선택/RNG/predicate/cache·초기ID·전체 unit_smoke/tooling PASS. 실시간44,368콜·fresh2,907: 감지 없음63.12%, 탄종/장비35.47%, 반경만0.31%, 선택1.10%. 예시27/누락44,341 exact. OFF 단발631.5초·AI max17,112us지만 첫 upgrade0.577초 | 진단만 채택. 전체 단발 gate는 upgrade 하한 FAIL로 보존·기본 승격 없음. 다음 감지의 거리/FOV/LOS 세분화, 반경/드랍 blanket 확대 금지. 단일 창의 호출 빈도를 부족 시간·인과/생존 개선으로 해석하지 않음 |
| E-075 | 구역 전환 전/후를 섞지 않고 보급 재고를 비교할 수 있는가? | 별도 stage2+1초 관측·초기ID exact·21종 오류 검증. 5배4창 중0.396초 지연 실패 보존, 전용 창1배로 분리. 실시간seed41001 쌍 지연0.0039/0.0065초·재고208/201·phase/인원/ID PASS | 관측 도구 채택, 가속/실시간 합산 금지·기본 후보 유지. 단일 phase 시점의 생존/빈 탄약8/3→9/1은 인과/5-run/수동 증거 아님. 회귀 첫 wall_traffic 실패 보존·독립/전체 재실행 PASS. 다음 실제 회복 검색 필터 진단 |
| E-074 | 비활성 진단의 ID 교란을 제거한 비교가 성능/보급을 보존하는가? | 지연 로딩·초기 ID 회귀·unit_smoke/tooling PASS. `3220d11` 비계측5+5 초기exact·평균696.0/750.5초·AI max22,638/26,514us·양군 gate PASS. 120초 빈 탄약24.2/25.0%, alive120/260 중앙26/17→28/17. 260초는4/5쌍 stage wave 전/후 불일치 | 도구 수정 채택, 게임플레이 opt-in 유지·기본 미적용. 수집/포기 방향성만 있고 부족/생존 개선 불충분, 후기 재고 비교 불가. 원시10판과 E-071 실패 보존. 다음 동일 phase 관측/회복 탐색 진단 |
| E-073 | 부모 소스와 E-071 후보의 비계측5+5는 보급/페이싱을 보존하는가? | 양군4-run씩 종료했지만 candidate probe의 eager 진단 Resource/RefCounted 생성이 OFF에서도 내부 ID를 소비했다. 첫 입력의60봇 모두 부모와 ID%8 엄폐 방향 값이 달랐음 | 비교 조건 교란으로 중단·원시8판 보존·승격 제외. ID를 지운 초기 필드 일치만으로 행동 중립을 주장하지 않는다. E-074 지연 로딩/ID exact 검증 후 새 배치 |
| E-072 | E-071의1.19초 AI 갱신을 구간별로 재현할 수 있는가? | 선택적7구간 계측·같은 total/최대 보존·4프레임 주기·합성6ms·전체 unit_smoke PASS. 계측 대조/후보 각2-run, 128,879표본에서50ms 초과0. 실시간 후보는261표본·재무장 관측16회·5초 초과 추적 후 재무장 사례 | 계측 채택, 원래 급증 원인 미확정·실패 보존. 실제 추적 계약만 확인했으며 보급/생존 승격 아님. 다음 E-073 비계측5+5로 분리 검증 |
| E-071 | 진행 중인 아이템 추적을 5초에 포기하지 않으면서 정체/과도한 추적은 막을 수 있는가? | `builds/verification/E071_chase`: 30m 대조5.1초 포기/후보8초 수집, 정체/15초 상한/도착/retarget/삭제/적 우선순위·전체 unit_smoke PASS. M1 후보759.0/648.8초·alive120/260 `26/16`·`28/18`, AI max1,187,585/24,256us로 합산 성능 FAIL | 기본 비활성 opt-in만 유지. 급증은 재실행에 없지만 원인 미확정으로 5+5 승격 중단. 첫 실패를 버리거나 gate를 완화하지 않으며 새로운 EXE/수동 판정은 없음 |
| E-070 | 탄약 없는 봇은 탐색/순찰/아이템 추적 중 어디에서 진행을 잃는가? | `builds/verification/E070_progress/window/run_1.json` 실시간 260초 261표본·최대 지연 0.012초 PASS. 빈 탄약 표본 1,557 중 RECOVER 831(탐색314/순찰452/엄폐65), IDLE490·CHASE56. 약 29→15m 접근 후 5초 경계에서 동일 탄약 재추적, 재무장 관측6회 | 도구만 채택. E-071은 진행 없는 정체와 정상 장거리 접근을 소형 재현으로 분리. 단발 창의 alive120/260 `17/8`은 생존 승격 근거가 아니며 5배 가속 결과와 합산 금지. 5배 전체 2-run의 지연 표본31/25개는 품질 FAIL로 보존 |
| E-069 | 빈 탄약 봇이 선점 도착 대기 때문에 파밍하지 않는가? | `builds/verification/E069_context` 기본 동작 1-run 689.3초·구조 PASS. v2 관측의 빈 탄약 봇 120/260초 `9/7` 중 IDLE+도착 대기 기하 충족 `0/0`. 초기 기존 필드 불변·거리/존/도착 계산 검증 PASS | 도구 계측만 추가. 대기 분기는 존재하지만 주원인 증거 없음. 다음은 탐색 실패 이유와 목표 진행을 분리하며 도착 전/회복 상태를 대기 중으로 오인하지 않음 |
| E-068 | 두 POI의 기존 소모품 한 슬롯을 첫 총과 호환되는 탄약으로 구성하면 유효 재고가 개선되는가? | `builds/verification/E068_repeat` 대조/후보 각 5-run 평균 705.9/713.1초, alive@120/260 중앙 `23/17`·`27/16`, T10 317.3/321.0초, 구조 PASS. 초기 5쌍의 총/위치/RNG/비대상 불변과 1-2슬롯 교체 계약 유지 | 생존 개선안으로 승격하지 않음·기본/EXE 미적용. 120초 후보 빈 탄약 48/127 표본 모두 필드에 호환 탄이 있으나 10m 내는 1/48·가장 가까운 거리 중앙 63.1m. 거리/탐색/이동 진단 우선; 전역 총량 부족이나 LOS/nav 접근 가능성의 증거가 아님 |
...
| E-058 | `survival_break`가 놓은 bot target을 defensive gunfire 중 다시 소유하지 않으면 lethal churn이 줄어드는가? | `C:\tmp\n2_play_11_e058_counterfire_no_target_ownership_pilot_20260829`: 716.4초, alive@60/120/260 `32/21/16`, T10 284.9초, survival death 25, episode fast reacquire `9/104`(전부 damage), stuck 상한 초과 | 메커니즘은 제거했지만 생존·구조 방향 gate 실패. 완전 revert·5-run 금지, target 소유/retreat grace 재혼합 금지. fast reacquire 단독 원인 폐기 |
| E-059 | behavior-neutral cover progression linkage가 선택-미도달 사망을 이동 부족·피격 시간·perception 소실로 분리하는가? | `C:\tmp\n2_play_11_e059_cover_progression_5run_20260901`: 평균 665.7초·scale PASS. 648 episode, 진행 관측 530건 평균 0.11; 사망 68건 cover `no/partial/reached=34/32/2`, 첫 피격 `<0.25/0.25-1/1s+=10/37/21`, perception 소실 24, counteraction 62 | schema v2 진단 채택, gameplay PASS 아님. 즉사·LOS·재획득 단독 대신 평균 12.98m cover와 선택 후 사망 1.62초의 reachability/fallback만 E-060 1-run 허용 |
| E-060 | lethal window 안에 닿는 6m cover만 고르고 없으면 scatter하면 무의미한 먼 cover 추적이 줄어드는가? | `C:\tmp\n2_play_11_e060_reachable_cover_fallback_pilot_20260901`: 486.6초, alive@60/120/260 `27/22/18`, survival death 21, cover selected/reached `8/0`, death not-selected 21/21, stuck 0.04/entity/min, stage3 없음 | hard gate 실패, 코드·테스트 완전 revert·5-run 금지. reachability cap/fallback 재조정 금지; 기존 cover commitment를 보존한 접근 이동만 다음 단일 후보 허용 |
| E-061 | 기존 cover/nav commitment를 유지하고 survival-break cover 접근만 1.25배로 하면 낮은 진행과 생존이 함께 개선되는가? | `C:\tmp\n2_play_11_e061_cover_approach_speed_125_pilot_20260901`: 536.0초, alive@30/60/120/260 `50/34/22/12`, T50/T10 `29.2/276.3초`, survival death 17, cover selected/reached `87/4`, 진행률 0.09, stuck 0.03/entity/min, stage3 없음 | hard gate 실패, 코드·테스트 완전 revert·5-run 금지. 속도 재조정 금지; `no_threat` 91/108·perception 소실 84/108·평균 exit 0.68초 근거로 선택 cover의 spatial commitment만 E-062 1-run 허용 |
| E-062 | survival-break가 perception 소실 뒤에도 이미 선택한 cover까지 이동을 마치면 조기 목표 폐기와 초기 붕괴가 줄어드는가? | `C:\tmp\n2_play_11_e062_spatial_cover_commitment_5run_20260901`: 평균 753.5초(576.3-881.9), alive 중앙 `52/39/26/19`, T10 322.8초, cover selected/reached `269/42`, 진행률 0.26, survival-state/DISENGAGE death rate `4.18/5.30`, 빠른 재획득 25.3%, stuck/disengage 0.02/0.17, scale PASS | 자동 후보 유지. 전체 `unit_smoke` PASS. episode death 84와 전체 DISENGAGE reengage 25.0%는 clean packaged 수동 3판 watch이며 수동 전 M1 승격 금지 |
