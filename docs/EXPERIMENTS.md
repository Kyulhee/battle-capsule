...
| ID | 질문 | 최신 증거 | 판단 |
|---|---|---|---|
| E-074 | 비활성 진단의 ID 교란을 제거한 비교가 성능/보급을 보존하는가? | 지연 로딩 수정, 실제 OFF/ON/loot 초기 ID 회귀·전체 unit_smoke·tooling PASS. 부모 `9f23dde`와 후보의 seed41000-41004 사전 실행은 초기 스냅샷/ID exact 일치, 후보 진단 미로드·미생성 확인 | 초기 중립성만 확인. 별도 출력의 비계측 대조/후보5+5를 진행하고 E-073 자료와 섞지 않는다. 기본 적용·패키지·수동 합격은 미판정 |
