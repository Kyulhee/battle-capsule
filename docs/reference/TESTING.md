...
`verify_audio_catalog_assets.gd`는 핵심 오디오 7종의 파일 존재, raw WAV 로딩, ID별 캐시 재사용과 catalog 누락 0을 요구한다. 무기음은 0.05-1.0초, AR -6dB, pitch variation ±2% 범위도 검사하며 채택된 발걸음 볼륨은 바꾸지 않는다.
