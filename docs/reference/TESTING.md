...
패키지의 제어된 정상 결과→RESTART→독립 재실행 저장 검증은 아래 전용 명령을 사용한다. 기본 E067의 EXE/PCK SHA는 고정되어 있다. 다른 패키지는 `--runtime-source package --package-dir`를 함께 지정하며 export manifest의 build/source·hash와 Git 소스 BuildInfo를 대조한다. 출력은 반드시 **새 `builds/verification` 하위 경로**여야 한다. 빈 host에서 독립 APPDATA를 입증한 뒤 `--main-pack`으로 패키지 코드를 실행한다. `probe_release_flow.gd`를 실제 사용자 프로젝트에서 직접 실행하지 않는다.
...
python tools/run_release_flow.py --runtime-source package --package-dir builds/playtest/E-091_b1ac590 --restart-count 5 --out-dir builds/verification/package_repeat_new
```
...
`--runtime-source` 기본값 `e067`은 기존 PCK, `workspace`는 현재 소스의 격리 실행이다. 후자는 src/project/data도 해시로 고정하고 source 모드를 증거에 명시한다. `package`는 명시한 PCK의 검증 후 실제 EXE도 같은 격리 APPDATA에서 headless 메뉴 부팅/120프레임 종료한다. release template의 경로 override 금지 때문에 외부 스크립트를 EXE에 붙이지 않으며, EXE smoke와 PCK의 제어 흐름 증거를 구분한다. 같은 메뉴 라벨이 보여도 workspace PASS를 패키지 PASS로 해석하지 않는다. 봇/플레이어 사망과 미션을 제어하는 자동 fixture이므로 자연 경기 완주·EXE 조작·수동3판·Forward+·장시간 soak를 대신하지 않는다.
