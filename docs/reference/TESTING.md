...
| `ai_test_arena` | AI 정책·테스트 맵·고정 스폰·격리 옵션 변경 | 결정/이동 정책 + 6개 preset + 실제 duel 공격 + high rock 횡단 + squad 표적 기억·접촉률 |
...
`duel_1`은 4.5m 고정 1대1과 초기 loot 격리, `rock_nav_1`은 Night 크기 high rock 양쪽 24m CHASE 횡단, `squad_4`는 다중 위협, `systems_8`은 loot 포함 8봇 상태 전이, `random_8`은 고정 스폰 의존성 비교용이다. Arena 통과는 재현·회귀 근거이며 Night gameplay 승격 근거가 아니다.
