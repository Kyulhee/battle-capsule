...
`verify_audio_catalog_assets.gd`는 path가 설정된 핵심 오디오 7종의 파일 존재, 0.05초 초과 길이, raw WAV 로딩, ID별 캐시 재사용과 catalog 누락 0을 요구한다.
...
`profile_runtime_performance.gd`는 headless가 아닌 Forward+ 실행에서 frame/process/physics/navigation, draw call, collision pair, pipeline compile, AI 비용을 JSON으로 남긴다. 같은 조건을 최소 2회 반복하며 `perf_hide_minimap=true`는 UI 병목 대조에만 쓴다. N2-ASSET-01까지의 기준은 p95 15.5-16.5ms, p99 20.7-26.6ms, 33ms 초과 0.18-0.23%다.
