...
| `ai_test_arena` | AI 정책·테스트 맵·고정 스폰·격리 옵션 변경 | 정책 + 5개 preset + 실제 duel 공격 + squad 3초 표적 기억 |
...
`squad_4`는 자연 행동을 2초 계측한 뒤 교차 피해, peripheral switch, combat-loot, sniper 최소 사거리를 강제 구간에서만 격리한다. 이후 3초 동안 네 봇이 player 표적을 유지하고 최소 세 봇이 공격해야 통과한다. 자연 다자전의 표적 전환이나 간격을 승격 근거로 사용하지 않는다.
