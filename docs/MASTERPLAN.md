...
- N2-MAP-06: `Central Meadow` 동쪽 바위는 동일 시드 60봇에서 open 피해 71.2→69.5%, loot-hub 피해 9.0→15.8%로 이동시켰지만 새 셀 정체가 0→15회 반복되어 폐기했다. `pacing_candidate`는 이제 명시적 맵 경로 없이는 실행되지 않는다
...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, spawn/목적지·open 피해 위치 계측, high rock ramp 제거, 45도 정렬 지도 | high rock 정체 원인 제거 뒤 60/99 raw stuck와 hotspot 감소. 99 disengage는 여전히 실패 | 최종 맵 승격, 자연스러운 병목 | 중앙 동쪽 엄폐 후보 단일 재검증 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | high rock ramp 제거로 정체 원인을 줄였고 이전 중앙 엄폐 실험의 재시도 조건이 생김 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | E-032 동쪽 엄폐 하나만 동일 시드로 재검증 |
