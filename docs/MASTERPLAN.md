...
> 최종 업데이트: 2026-07-22. 남은 작업은 마일스톤 -> 트랙 -> 종료 조건 -> 다음 slice 순서로 관리한다.
...
| loot/economy | initial non-pistol pool 제어, first-upgrade source telemetry, stage/supply upgrade 흐름 | source/context 구조 유지. 기존 초 단위는 재측정 대상 | 무기 티어·방어구 다양화, broad economy cut | 코어 압력 통과 뒤 M3 후보 설계 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 지역 표현은 개선, 표면 속도와 다음 존 선점 후보 적용 | 도로의 빠른 노출과 숲의 느린 엄폐, 선점 조우를 플레이에서 설명 가능 | N2-PLAY-09 |
...
- 무기 티어는 같은 무기 종류의 단순 수치 복제가 아니라 획득 경로·위험·식별성을 함께 설계한다. N2-PLAY-09와 페이싱 기준 통과 전에는 구현하지 않는다.
