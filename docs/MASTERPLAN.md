...
> 최종 업데이트: 2026-07-17. 남은 작업은 마일스톤 -> 트랙 -> 종료 조건 -> 다음 slice 순서로 관리한다.
...
- 최신 gameplay slice: N2-PACE-43 초기 pickup 3.5m 간격 후보 폐기
- 최신 map/world slice: N2-MAP-03 비충돌 world route cue와 역할별 MultiMesh 배치
...
- N2-MAP-02: 6개 route를 역할별 공유 style로 minimap/fullmap에 표시하고 full/map 240px 캡처를 `visual_review`에 추가. `unit_smoke`, `visual_review` 통과
- N2-MAP-03: 6개 route를 폭 0.26-0.36m의 비충돌 world strip으로 연결하고 127개 cue를 역할별 4개 MultiMesh로 배치. 방향/폭/충돌 smoke와 조감·플레이 시점 캡처 통과
...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. 미세 AI 예외와 primary route cover 추가 모두 생존 이득보다 pathing 회귀가 컸다. map과 world의 route 표시는 연결됐으므로 다음은 실제 조작에서 route/cover 판독과 cue 강도를 확인한다.
...
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot damage, nav unstick, inside-edge zone release | post-kill 획득을 절반 이하로 줄여도 stage1 사망 유지 | 완전한 flashlight/fear/battery 전술 | AI 예외 추가 중단, 구조 증거 우선 |
...
| 맵/경로 | night forest candidate, POI/route telemetry, minimap/fullmap/world route 표시 | 6개 역할 route가 map UI와 비충돌 world strip으로 연결됨 | 최종 맵 승격, hotspot 수동 검증, 실제 route 이동 계약 | 첫 1분 route/cover 판독 |
| 화면/UI | HUD, minimap, inventory, pickup label/glow, Night 월드 가독성 프로필 | deterministic 캡처에서 cover·수풀·지면 윤곽과 픽셀 대비 gate 통과 | 실제 플레이 전 구간 가독성 승격 | 수동 first-minute read |
...
| T3 야간 가독성 | 플레이어/픽업/cover/route가 실제 화면에서 읽힘 | Night/map/world cue 자동 캡처 통과, 실제 조작은 미확정 | `visual_review` 캡처와 수동 기록에서 route/cover 판독 가능 | first-minute read |
| T4 맵/경로 체감 | route choice가 bot collision보다 강하게 느껴짐 | map UI와 world route 연결 완료, 이동 권한은 없음 | 주요 POI/route가 플레이 중 구분되고 stuck hotspot이 gate 안에 있음 | first-minute route 선택 기록 |
| T5 자산 승격 | gameplay를 돕는 자산만 런타임으로 승격 | bush와 핵심 icon/audio 일부만 통합 | 자산이 가독성/피드백 문제를 실제로 줄임 | route/POI landmark prop 후보 audit |
...
1. T1이 최우선이지만 같은 미세 AI lever나 route cover 추가를 반복하지 않는다. route를 선택 정보로 보이게 하고 수동 첫 1분을 확인한 뒤 페이싱 후보로 돌아온다.
