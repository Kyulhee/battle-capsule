...
- 최신 map/world slice: N2-MAP-14 West Ridge 감시 능선, 노출 도로·숲 우회와 AI 앵커
...
| 맵/경로 | 260m 후보, Cabin Row 3입구 정착지, West Ridge 감시 능선, POI 내부 반경과 실제 landmark 지도 | Cabin 세 입구와 Ridge 도로/숲 두 경로 NavMesh, identity·캡처·60봇 성능 통과 | 최종 맵 승격, 두 지역의 수동 정체성·병목·압력 판정 | N2-PLAY-08 두 지역 판정 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | Cabin Row 정착지와 West Ridge 감시 능선의 물리·AI 문법 완료 | 지도 원 없이 두 지역을 구분하고 서로 다른 접근·교전 선택을 설명 가능 | N2-PLAY-08 |
