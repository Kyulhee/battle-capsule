...
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | 올바른 월드 60/99봇 normalized stuck 0.01/약 0. 99봇 disengage 0.42는 실패 유지 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | 60봇 Forward+ 재기준화 뒤 수동 기준 확인 |
| 전투/AI | bot doctrine, opening guard, player 위협/5초 기억, 순수 결정·이동 정책, player-target 국소 분산, 분산 갱신·단기 검색 캐시, bot damage, nav unstick | squad 유지율 100%, 강제 근접 0.2초 분리. 올바른 월드 AI 평균 60/99봇 317.9/435.6µs | Night 근거 효용 가중치, 완전한 flashlight/fear/battery 전술 | `xlarge_60` Forward+ 성능 재측정 |
...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, open 피해 계측, 실제 geometry nav bake, 96m Arena traffic, 45도 정렬 지도 | Minimap/TestMap 숨은 충돌체 제거. open 4봇 stuck 0, wall stuck 4, 제품 60/99봇 stuck 1.4/1.2 | 최종 맵 승격, 자연스러운 병목 | 성능·수동 기준 확인 전 장애물 추가 보류 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 실제 navmesh와 올바른 260m 물리 월드를 복구해 60/99봇 stuck 1.4/1.2. 위치 피해 분포는 새 기준선 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | 성능·수동 기준 확정 뒤 한 개 맵 후보만 재검증 |
