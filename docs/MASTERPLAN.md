...
- 최신 gameplay slice: N2-AI-01 플레이어 표적 위협 판정 분리
...
| 전투/AI | bot doctrine, opening guard, player-target 위협 분리, bot damage, nav unstick | player 표적은 무관한 방관자 때문에 이탈하지 않음. bot-only 판정은 보존 | 위협 점수/commitment 계층, 완전한 flashlight/fear/battery 전술 | 새 맵 후보에서 수동 압박 재검증 |
...
| 맵/경로 | night forest candidate, POI/route telemetry, 45도 정렬 minimap/fullmap | 180m/30초 횡단, 장애물 4.2%, 8봇 수동과 99봇 자동 표면 불일치 | 최종 맵 승격, 자연스러운 병목, 대표 수동 preset | 규모·물리 밀도 후보 설계 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 180m·장애물 4.2% 기준이 수동 체감과 불일치 | 주요 POI가 물리 동선으로 구분되고 stuck hotspot이 gate 안에 있음 | 1차 확장 whitebox 후보 |
