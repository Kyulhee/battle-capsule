...
- 최신 gameplay slice: N2-PACE-36 bot-only simulation participant 기준선
...
- 판단: observer 분리로 ATTACK 이상치는 최대 16.0초로 정상화됐지만 duration/stuck gate 실패로 v5는 비기본 후보 유지

## Night BR 페이싱 판정 창

| 지표 | 목표 창 | 현재 | 판단 |
|---|---|---|---|
| first upgrade | 120-300초 | v5 222.8초 | 통과 |
| stage2 | 240-420초 | v5 220.1초 | 이른 편 |
| stage3 | 540-720초 | v5 590.1초 | 통과, 1/5 run 도달 |
| match duration | 600-900초 | v5 평균 434.7초 | 실패 |

첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. bot-only v5는 첫 획득 6.3초, 첫 접촉 7.0초, run당 stage1 사망 94-97명이다. 비충돌 acquisition 유예만 늘리면 봇이 이동 중 모인 뒤 4초 hard-bump를 통해 전투가 재개됐다. 다음은 `ZONE_ESCAPE`가 반경 95%에서 시작해 75%까지 안쪽으로 수렴하는 경로가 이 충돌을 만드는지 확인한다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, pacing preset, runtime combat tuning, bot-only simulation participant | observer 분리 5-run 완료. v5 평균 434.7초, spawn 99/99 | 99명 default 승격, release 기준선, v5 기본 승격 | `ZONE_ESCAPE` 수렴 영향 분리 |
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot-only damage multiplier, nav unstick override | guard 6배 후보가 hard-bump 5/5, stuck 0.26으로 회귀 | 완전한 flashlight/fear/battery 전술 | 초반 가장자리 복귀와 collision acquisition 분리 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | bot-only v5 평균 434.7초, 범위 271.0-655.5초 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | `ZONE_ESCAPE` 수렴 영향 분리 |
| T2 오프닝 체감 | 첫 1분이 즉사/랜덤 충돌처럼 느껴지지 않게 함 | guard 연장은 hard-bump 충돌로 우회되고 duration/stuck이 악화 | opening 변경이 duration gate를 깨지 않고 수동 플레이에서 납득됨 | 이동 분리 후보 뒤 first-minute playtest |
...
1. T1을 먼저 안정화한다. bot-only v5 기준선에서 `ZONE_ESCAPE`의 긴 안쪽 복귀가 first acquisition과 stage1 소모에 미치는 영향을 분리한다.
