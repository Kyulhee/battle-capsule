...
- N2-NAV-01/02: high rock climb ramp를 제거한 뒤에도 runtime navmesh가 `polygons=0, vertices=0`인 채 direct fallback으로 이동하는 근본 결함을 확인했다. WorldBuilder geometry를 source group으로 bake하자 rock 횡단 stuck 1→0, 60/99봇 stuck 39.0→2.8·53.2→12.4, AI 평균 341.9→260.7µs·473.0→353.6µs로 개선됐다
...
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | 실제 nav 60/99봇 normalized stuck 0.01/0.03. 99봇 disengage 0.45는 실패 유지 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | 잔여 nav hotspot 뒤 수동 기준 확인 |
...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, open 피해 계측, 실제 geometry nav bake, 96m Arena traffic, 45도 정렬 지도 | 실제 nav open 4봇 stuck 0, 강제 wall stuck 4. 제품 99봇 잔여 정체 59.7%가 `0,40` | 최종 맵 승격, 자연스러운 병목 | `0,40` ZONE_ESCAPE hotspot 격리 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 빈 navmesh를 실제 geometry bake로 교정해 60/99봇 정체를 크게 줄였다. 강제 wall과 `0,40` 잔여 정체는 분리 대상 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | N2-NAV-03 `0,40` 격리 뒤 한 개만 맵 재검증 |
