...
- 최신 gameplay slice: N2-PERF-01 60봇 AI 갱신 분산·검색 캐시·텔레메트리 배치
...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, POI/route telemetry, 45도 정렬 지도 | 확장 후보는 빈 공간을 줄였지만 open 전투 66.7%, stuck 49회로 흐름 실패 | 최종 맵 승격, 자연스러운 병목 | AI 위치 효용 연결 뒤 whitebox 재검증 |
...
| T2 오프닝 체감 | 첫 1분이 즉사/무접촉 양극단이 아니게 함 | 수동 조우·간격·추적·사망 압박은 적절, 자동 성능 개선 완료. 확장 자동은 첫 조우 7.1초 과밀 | 성능 재확인과 자동 duration이 함께 납득됨 | `xlarge_60` 성능 체감 재확인 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 260m 후보가 공간 밀도는 개선했지만 open 피해 66.7%, stuck 49회 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | AI 위치 효용 뒤 2차 whitebox |
