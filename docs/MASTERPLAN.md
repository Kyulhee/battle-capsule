...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, spawn/목적지 POI 계측, 45도 정렬 지도 | 중앙 바위는 loot-hub 전투를 늘렸지만 새 정체 셀을 만들어 폐기 | 최종 맵 승격, 자연스러운 병목 | open 전투 셀·인접 POI 계측 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 중앙 거점 엄폐는 전투 위치를 바꿨지만 국소 정체를 만들었고 정확한 open 누출 위치는 미계측 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | open 피해 셀·인접 POI를 계측한 뒤 한 경계만 재설계 |
