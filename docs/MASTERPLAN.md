...
> 최종 업데이트: 2026-07-20. 남은 작업은 마일스톤 -> 트랙 -> 종료 조건 -> 다음 slice 순서로 관리한다.
...
- 최신 map/world slice: N2-MAP-12 명시적 엄폐 계약과 Cabin Row 3방향 compound
...
| 맵/경로 | 260m 후보, 실제 nav bake, 45도 지도, POI 목적지, 지면 구역, 명시적 엄폐, Cabin Row 3입구 compound | 실제 캡처, 엄폐 물리 smoke, 세 입구 NavMesh 경로, 60봇 p95 12.71-12.86ms 통과 | 전 지역 고유 실루엣, 최종 맵 승격, 자연스러운 병목·압력 | N2-PLAY-06에서 진입·우회·시야 전환 판정 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | Cabin Row에 물리·AI 문법 연결. 전체 지도는 아직 내부 POI 반경을 원으로 표시 | 지도 원 없이 landmark 무더기·지면·엄폐·AI 목적만으로 주요 지역을 구분하고 수동 체감 통과 | N2-PLAY-07 -> N2-MAP-13 -> West Ridge Watch Post |
