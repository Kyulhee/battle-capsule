...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. 그러나 guard 연장, `ZONE_ESCAPE` 복귀 분리, loot 공유 진단, 12초 비전투 근접 분산까지 stage1 사망 95명대를 바꾸지 못했다. opening 전용 예외를 더 쌓지 않고, 다음은 stage1 bot끼리의 damage만 제한해 더 많은 생존자가 stage2에 도달하도록 한다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | v6 spawn 99/99, stuck 0.14. duration 평균 465.1초 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | stage1 damage shaping 후보 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | stage1 사망이 v6/v7 모두 평균 95.6명 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | stage1 bot damage shaping |
...
1. T1을 먼저 안정화한다. v6의 stage2 이후 전투는 유지하고 stage1 bot끼리의 damage만 낮춰 생존 분포를 바꾼다.
