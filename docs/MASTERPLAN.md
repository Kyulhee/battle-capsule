...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. 미세 AI 예외와 primary route cover 추가 모두 생존 이득보다 pathing 회귀가 컸다. 현재 route는 분류 데이터일 뿐 이동을 유도하지 않으므로, 다음은 minimap/fullmap에서 선택 가능한 route로 읽히게 한 뒤 수동 첫 1분을 본다.
...
| 맵/경로 | night forest candidate, POI/route telemetry, minimap/fullmap 데이터 | canonical v6 킬 flank 46.5%, off-route 33.4%. route는 현재 분류 전용 | 최종 맵 승격, hotspot 수동 검증, 실제 route 이동 계약 | minimap/fullmap route 가시화 |
...
| T4 맵/경로 체감 | route choice가 bot collision보다 강하게 느껴짐 | route는 분류 전용이고 primary cover 추가는 새 stuck hotspot 생성 | 주요 POI/route가 플레이 중 구분되고 stuck hotspot이 gate 안에 있음 | minimap/fullmap route 가시화 |
