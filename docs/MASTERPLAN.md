...
- 최신 자동 결과: avg duration 689.0초, 범위 336.2-1219.9초, first contact 13.6초, first upgrade 285.5초, stage2 283.4초, stage3 654.2초

## Night BR 페이싱 판정 창

| 지표 | 목표 창 | 현재 | 판단 |
|---|---|---|---|
| first upgrade | 120-300초 | 285.5초 | 유지 |
| stage2 | 240-420초 | 283.4초 | 유지 |
| stage3 | 540-720초 | 654.2초 | 유지 |
| match duration | 600-900초 | 평균 689.0초, 범위 336.2-1219.9초 | 평균 통과, 분산 실패 |

첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. v5로 평균 match duration은 확보했지만 조기 종료와 과도한 장기전을 함께 줄여야 한다. 분산을 안정화한 뒤 수동 플레이에서 오프닝과 route/cover 판독을 함께 확인한다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, pacing preset, runtime combat tuning, `run_verify.py` profile | v4 기준선, v5 평균 689.0초 자동 PASS | 99명 default 승격, release 기준선, v5 기본 승격 | T1 조기/장기 종료 분산 원인 분리 |
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot끼리만 적용되는 damage multiplier | v5 first contact 13.6초, player damage 불변 unit PASS | 완전한 flashlight/fear/battery 전술 | duration 분산 안정 뒤 첫 1분 수동 판정 |
| loot/economy | initial non-pistol pool 제어, first-upgrade source telemetry, stage/supply upgrade 흐름 | first upgrade 285.5초 | broad weapon chance cut, broad economy cut | upgrade band를 깨지 않는 분산 조정 |
| zone/pacing | stage2/stage3 timing, zone pressure telemetry | stage2 283.4초, stage3 654.2초, zone death 1회 | 안정된 10-15분 분포 | 조기/장기 종료 run 비교 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | v5 avg 689.0초, 범위 336.2-1219.9초 | 3-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | 조기/장기 종료 원인 비교 후 좁은 분산 lever 적용 |
| T2 오프닝 체감 | 첫 1분이 즉사/랜덤 충돌처럼 느껴지지 않게 함 | v5 first contact 13.6초, 5초 brush는 실패 | opening 변경이 duration gate를 깨지 않고 수동 플레이에서 납득됨 | T1 이후 first-minute playtest |
...
1. T1을 먼저 안정화한다. 평균만 보고 v5를 승격하지 않고 조기 종료와 20분 이상 run의 원인을 분리한다.
