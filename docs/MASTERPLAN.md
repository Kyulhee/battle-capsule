...
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | 올바른 월드 60/99봇 normalized stuck 0.01/약 0. 99봇 disengage 0.42는 실패 유지 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | N2-MAP-10 공간 흐름 감사 |
...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, open 피해 계측, 실제 geometry nav bake, 96m Arena traffic, 45도 정렬 지도 | Minimap/TestMap 숨은 충돌체 제거. open 4봇 stuck 0, wall stuck 4, 제품 60/99봇 stuck 1.4/1.2 | 최종 맵 승격, 자연스러운 병목 | 빈 공간·픽업·물리 병목 감사 뒤 후보 1개 |
| 화면/UI | HUD, minimap, inventory, pickup label/glow, Night 월드 가독성, landmark-only 전체맵 라벨 | 정적 Minimap 1회 캐시와 동적 존·player 분리, 방향 캡처 통과, 60봇 p95 15.4-15.8ms | 실제 플레이 전 구간 가독성 승격 | 다음 수동 실행에서 지도·first-minute read |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 실제 navmesh·물리 월드·60봇 성능 복구. 위치 피해 분포와 사용자 평지 피드백이 새 기준선 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | N2-MAP-10 감사 뒤 한 개 비기본 후보만 재검증 |
