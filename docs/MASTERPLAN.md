...
> 마지막 업데이트: 2026-04-25 (v0.5 계획 반영)  
> 이 문서는 AI 에이전트 간 인수인계 및 장기 방향 공유를 위해 작성되었습니다.

**현재**: v0.4 완료 — 다음: v0.5 (전장 경제 — 자기장 강화, 쉴드 너프, 드랍 분리, 힐 2단계)
...
> 그룹 전술(CoverRegistry/플랭킹/포위) 항목은 방향 재검토 후 제외. 봇마다 독립 판단을 유지하되, 같은 목표를 공격 중인 봇 수에 따라 공격성 파라미터를 상향하는 방식으로 v0.8에 일반화.

### v0.8 — 집단 공격 강화 + 난이도 시스템

#### 집단 공격 강화

봇마다 글로벌 상태 없이, 같은 목표를 공격 중인 봇 수를 인식해 공격성을 자연스럽게 높인다.

- **공격자 수 감지**: ATTACK 상태 진입 시 `"bots"` 그룹 순회 → `target_actor == self.target_actor`인 봇 수 집계
- **공격성 스케일**: 공격 봇 수 ≥ 2이면 `fire_rate` 단축 + DISENGAGE 진입 임계값 상향 (혼자라면 쉽게 물러서지 않음)
- **접근각 분산**: 이미 구현된 scatter offset을 유지 — 인위적 포위 없이 자연스럽게 다른 방향에서 접근
