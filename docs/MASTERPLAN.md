...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. guard, zone 복귀, loot 공유, 근접 분산, 피해, post-kill 재획득을 각각 제한해도 stage1 사망은 92-96명에 고정됐다. 미세 AI 예외를 더 쌓지 않고, 다음은 `visual_review`와 route hotspot을 함께 읽어 구조적 encounter density 후보를 고른다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | v6 spawn 99/99, stuck 0.14. v7 미세 lever 두 개 연속 실패 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | 구조적 route/encounter 후보 대기 |
...
| zone/pacing | canonical match clock, nav bake 대기, bot-only participant, stage timing | v6 465.1초, 피해/post-kill v7 모두 gate 실패 | 안정된 10-15분 분포 | visual route read 뒤 구조 후보 재설계 |
| 맵/경로 | night forest candidate, POI/route telemetry, minimap/fullmap 데이터 | v6 kill route flank 47.7%, off-route 38.1%로 설계 route 밖 교전 우세 | 최종 맵 승격, hotspot 수동 검증 | open/off-route encounter audit |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | micro lever 6종에도 stage1 사망 92-96명 고정 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | N2-VIS-01 뒤 구조적 encounter density 후보 |
...
| T3 야간 가독성 | 플레이어/픽업/cover/route가 실제 화면에서 읽힘 | N2-VIS-01 캡처 대비 gate 통과, 실제 장시간 플레이는 미확정 | `visual_review` 캡처와 수동 기록에서 route/cover 판독 가능 | first-minute 수동 read |
| T4 맵/경로 체감 | route choice가 bot collision보다 강하게 느껴짐 | v6 kill의 85.8%가 flank/off-route, visual run도 open/off-route 우세 | 주요 POI/route가 플레이 중 구분되고 stuck hotspot이 gate 안에 있음 | open/off-route 구조 audit |
...
1. T1이 최우선이지만 같은 미세 AI lever를 반복하지 않는다. N2-VIS-01에서 route/cover 흐름을 확인한 뒤 구조적 encounter density 후보로 돌아온다.
