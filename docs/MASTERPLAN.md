...
- 자동 페이싱 기준: `night_br_m1_60` 5-run 평균 727.0초, 개별 654.3-829.5초
- 존 기준: 첫 축소 120초, stage2 260초, stage3 540초. first upgrade 평균 262.2초
...
- N2-SURV-01: 첫 축소를 190→120초로 당기되 stage2/3 260/540초를 보존했다. 구급상자 1개·50HP·전투 6초 잠금, 실드 픽업 전투 잠금, 붕대 전투 사용, HP 50%/25% 이동 92%/82%를 적용했다. 5-run 평균 727.0초, 범위 654.3-829.5초, first upgrade 262.2초, 정체 0.01/개체·분으로 자동 gate를 통과했다

## Night BR 페이싱 판정 창

| 지표 | 목표 창 | 현재 | 판단 |
|---|---|---|---|
| first upgrade | 120-300초 | 262.2초 | 자동 통과, 초기 총기 체감은 늦음 |
| stage2 | 240-420초 | 260.0초 | 자동 통과 |
| stage3 | 540-720초 | 540.1초 | 자동 통과 |
| match duration | 평균 600-900초, 개별 480-960초 | 평균 727.0초, 654.3-829.5초 | 자동 통과, 새 생존 제약 수동 확인 필요 |
...
|---|---|---|---|---|
| 매치/preset | `night_br_m1_60` 일반·수동·자동 공통 표면, bot-only simulation participant | 5-run 평균 727.0초와 개별 상·하한 gate 통과 | 생존 제약 수동 체감, 99명 default 승격 | N2-SURV-01 수동 회귀 |
...
| loot/economy | initial non-pistol pool 제어, first-upgrade source telemetry, stage/supply upgrade 흐름 | first upgrade 평균 262.2초, 누락 0/5. 후반 무기는 충분하지만 초반 체감은 부족 | 중심·도로/외곽 보상 차이, 별도 장착 방어구 | N2-EQUIP-01 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | 120초 첫 축소 후보가 자동 5-run 상하한 통과 | 실제 플레이에서도 지나친 공백·즉사·후반 지연이 없음 | N2-SURV-01 수동 회귀 |
| T2 오프닝 체감 | 첫 2분에 이동·파밍·교전 선택이 생김 | 자동 첫 접촉 7.1초·첫 킬 21.4초, 수동은 초기 총기 부족과 이동 부담 부족 | 지역별 초기 보상이 무작위 즉사 없이 이동 이유를 만듦 | N2-EQUIP-01 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 세 장소와 상황 기반 목적지·경로·선점 자동 계약, 수동 지역 구분 개선 | 지역 보상과 구조물 군집이 도로·숲·선점 압력을 함께 만듦 | N2-EQUIP-01 → N2-MAP-17 |
...
- N2-EQUIP-01은 지역별 초기 무기 접근과 실드/장착 방어구 분리까지만 다룬다. 다수 티어와 부위별 장비는 M2 후반 후보로 둔다.
- 무기 변형은 단순 수치 복제가 아니라 획득 지역·위험·화면 식별성을 함께 가져야 한다.
- 방어구는 용량 증가 반복보다 피해 감소와 이동·소음 trade-off가 분명한 소수 archetype부터 시작한다.

## 우선순위 원칙

1. 생존·자기장 자동 gate는 통과했다. 다음은 global loot 확률이 아니라 지역별 보상 차이를 구현한다.
