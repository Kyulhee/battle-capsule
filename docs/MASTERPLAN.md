...
- 최신 AI slice: N2-AI-09 다음 안전구역 entry/outer 선점·유지와 성향별 물리 도로 경유
- 최신 map/world slice: N2-MAP-15 도로 100%·일반 지면 92%·숲 84% 이동 경제
...
| 지표 | 목표 창 | 현재 | 판단 |
|---|---|---|---|
| first upgrade | 120-300초 | 과거 v6 224.4초 | M1 표면 재측정 필요 |
| stage2 | 240-420초 | 과거 v6 220.1초 | M1 표면 재측정 필요 |
| stage3 | 540-720초 | 과거 v6 590.1초, 1/5 run | M1 표면 재측정 필요 |
| match duration | 600-900초 | 과거 v6 평균 465.1초 | 실패, M1 표면 분포 없음 |
...
|---|---|---|---|---|
| 매치/preset | `night_br_m1_60` 일반·수동·자동 공통 표면, bot-only simulation participant | 무인자 Main runtime이 260m·60봇·loot 200 계약 통과 | 10-15분 canonical 분포, 99명 default 승격 | South Creek·AI 효용 뒤 5-run |
| 전투/AI | bot doctrine, player 위협/5초 기억, 국소 분산, IDLE POI 이동, 다음 존 접근 선점 | 근접·squad 회귀와 실제 Night 선점·유지 runtime 통과 | 도로 교통량과 선점 조우 수동 승격, flashlight/fear/battery 전술 | N2-PLAY-09 이동 압력 판정 |
| loot/economy | initial non-pistol pool 제어, first-upgrade source telemetry, stage/supply upgrade 흐름 | source/context 구조 유지. 기존 초 단위는 재측정 대상 | 거점 보상 차이를 만드는 최소 무기 변형·방어구 archetype | 코어 압력 통과 뒤 M2 범위 확정 |
| zone/pacing | canonical match clock, nav bake 대기, bot-only participant, stage timing, 선축소 점유 | v6 465.1초, next-zone 접근 선점 runtime 통과 | 안정된 10-15분 분포와 선점 체감 | N2-PLAY-09 뒤 분포 재측정 |
| 맵/경로 | 260m 후보, Cabin Row·West Ridge·Logging Ford, 실제 landmark 지도, 표면 이동 경제와 road waypoint | 세 장소와 빠른 도로·느린 엄폐 숲·동쪽 나루 NavMesh 통과 | 최종 맵 승격, 도로 노출과 숲 엄폐의 수동 선택 판정 | N2-AI-10 뒤 수동 판정 |
| 화면/UI | HUD, 120m 로컬 minimap, `M` 전체 지도, inventory, pickup label/glow, Night 가독성 | 단일 미니맵·768px 정적 캐시, POI 원 없는 전체 지도 방향/캡처 통과 | 지역 이동감 수동 승격 | N2-PLAY-08 두 지역 판정 |
| 자산/audio | 실사 총성·칼 분리음·발걸음, Cabin Row와 West Ridge GLB | catalog 누락 0, 프롭 수량·엄폐 계약, 60봇 p95 13.14-13.15ms | 칼 재청취, 검증되지 않은 나머지 지역 자산 | N2-PLAY-08 지역 판정 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | 기준 표면은 통합됐지만 최신 공간·AI 변경의 5-run 분포 없음 | `night_br_m1_60` 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족 | South Creek·AI 효용 뒤 재측정 |
| T2 오프닝 체감 | 첫 1분이 즉사/무접촉 양극단이 아니게 함 | 근접 대응을 보존한 채 원거리 교전 합류 포화 적용. 60봇 3-run 평균 216.7초 | 교전 품질을 보존하며 이동 압력과 과소모가 함께 납득됨 | N2-PLAY-07 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | Cabin Row·Watch Post·Logging Ford 세 장소와 도로/숲/병목 자동 계약 통과 | 세 장소를 다른 선택으로 이동하고 선점 조우를 수동 플레이로 설명 가능 | N2-AI-10 전략 효용 |
| T5 자산 승격 | gameplay를 돕는 자산만 런타임으로 승격 | 총성·발걸음, 칼 분리음, tree·landmark 후보 | 오디오 재청취 통과와 실제 피드백 문제 감소 | N2-PLAY-06 오디오 판정 |
