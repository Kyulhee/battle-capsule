...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, spawn/목적지 POI 계측, 45도 정렬 지도 | pickup 목적지는 POI/route를 소비하지만 open 피해 70-71%. `Central Meadow` 단일 run IDLE loot 37.7% | 최종 맵 승격, 자연스러운 병목 | `Central Meadow` 단일 whitebox |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | IDLE loot 목표 66-67%가 POI 내부지만 open 피해 70-71%. 중앙 거점 수렴이 가장 큼 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | 중앙 거점 입구·엄폐·개방 경계 단일 검증 |
