...
> 마지막 업데이트: 2026-05-06 (v1.6.1 릴리즈 후보, v1.7 로드맵 확정)
> 이 문서는 AI 에이전트 간 인수인계 및 장기 방향 공유를 위해 작성되었습니다.

## 현재 상태

**현재**: v1.6.1 릴리즈 후보 — 다음: v1.7 (AI Doctrine Hierarchy Refactor)
...
v1.6.1 기준 ATTACK 상태는 `CombatPlan`을 사용한다. 전술 ID는 `STRAFE / ADVANCE / KITE / PEEK_COVER / REPOSITION / HOLD_ANGLE`이며, 다음 v1.7에서 `BotDoctrine` 계층으로 분리한다.
...
### v1.6.1 — Release Stabilization `S`
...
### v1.7 — AI Doctrine Hierarchy Refactor `M`

**한 줄 요약**: 봇 전술을 코드 곳곳의 조건문에서 분리해 디버깅 가능한 계층형 프로파일로 만든다.

**새 구조**
- `Bot.gd`: 상태 머신, 이동/사격 실행, 센서 입력 수집만 담당.
...
3. Difficulty Overlay — 시야, 반응속도, 사주경계, aim spread, 업데이트 빈도.
4. Archetype Overlay — AGGRESSIVE/DEFENSIVE/SNIPER/OPPORTUNIST별 성향 보정.
...
### v1.8 — Complex Artifacts `M`

**한 줄 요약**: 2차 아티팩트로 반복 플레이 변수를 늘린다.
...
|---|---|
| 맵 2개 이상 | ❌ (v2.2 이후) |
| 아티팩트 20개 이상 | ❌ (v1.5+v1.8 합산 약 18개) |
| 미션 20개 이상 | ❌ (v1.4 보너스 15종 + 압박 17종은 성격 상이, v1.10 이후 재검토) |
| 난이도 4종 | ✅ |
...
> Phase 1(v1.6.1~v1.10) 완료 후 진행. 각 버전 착수 전 선행 조건 확인 필수.

### v2.0 — Map System Refactor `M`

**선행 조건**: 없음 (v1.10 완료 후 바로 시작 가능)
...
**작업 시작 전 확인할 파일**
- `Bot.gd` — AI 상태 머신, CombatPlan, NavigationAgent3D
...
**현재 미해결 이슈**
- ObjectDB leak 경고가 릴리즈 전 검증에서 재현되면 `--verbose`로 상세 확인 후 Timer/Node leak 여부를 정리한다.
- v1.7에서 Bot 전술 조건을 `BotDoctrine` 계층으로 분리해 아키타입별 보정을 디버깅 가능하게 만든다.
