...
> 최종 업데이트: 2026-07-18. 남은 작업은 마일스톤 -> 트랙 -> 종료 조건 -> 다음 slice 순서로 관리한다.
...
- 최신 gameplay slice: N2-PLAY-02 `xlarge_60` AI 전투·성능 수동 통과
- 최신 map/world slice: N2-MAP-05 260m 확장 whitebox와 실제 후보 검증
...
- N2-PERF-01: perception/sensory 갱신 위상을 분산하고 표적·픽업·후퇴 위협·압력 문맥을 0.10-0.25초 캐시했다. 교리 위치 텔레메트리는 누적 후 0.25초마다 기록하고 nav target은 0.35m 이동·상태 전환·stuck 복구 때 갱신한다. 60봇 프레임 p95 45.5→13.5ms, 33ms 초과 12.8→0.04%, 동일 seed 99봇 AI 평균 476→353µs. stuck 0.15→0.16으로 대조 범위
...
- N2-MAP-06/08: `Central Meadow` 동쪽 high rock은 최초 60봇 셀 정체 0→15회로 폐기했다. ramp 제거 뒤 1대1 실제 맵 횡단 5/5 통과에도 동일 seed open 피해 69.7→69.9%, 셀 정체 0→5회와 별도 seed 14회가 재발해 재배치를 종료했다
...
- N2-NAV-01: high rock climb ramp가 Arena 24m 횡단을 14.19m에서 막고 stuck 3회를 만들었다. ramp 제거 뒤 5/5회 2.8-3.0m까지 횡단·stuck 1회, 60봇 두 rock 셀 32→15회와 대표 duration 224.3→225.1초, 99봇 74→22회와 raw stuck 64.8→53.2를 확인해 채택했다
...
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | v6 spawn 99/99, stuck 0.14. 미세 lever와 primary cover 후보 실패 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | route 선택 표면 뒤 수동 기준 확인 |
| 전투/AI | bot doctrine, opening guard, player 위협/5초 기억, 순수 결정·이동 정책, player-target 국소 분산, 분산 갱신·단기 검색 캐시, bot damage, nav unstick | squad 유지율 100%, 강제 근접 0.2초 분리. 60봇 p95 13.5ms, 99봇 AI 평균 -25.9% | Night 근거 효용 가중치, 완전한 flashlight/fear/battery 전술 | `xlarge_60` 성능 체감 재확인 |
...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, spawn/목적지·open 피해 위치 계측, high rock ramp 제거, 45도 정렬 지도 | ramp 제거는 기존 hotspot을 줄였지만 high rock 추가 배치는 다중 traffic 정체 재발 | 최종 맵 승격, 자연스러운 병목 | 비-rock 엄폐 primitive를 Arena 다중 traffic에서 비교 |
| 화면/UI | HUD, minimap, inventory, pickup label/glow, Night 월드 가독성, landmark-only 전체맵 라벨 | deterministic 캡처에서 cover·수풀·지면 윤곽, 지도 방향과 라벨 혼잡 개선 | 실제 플레이 전 구간 가독성 승격 | `xlarge_60` 수동 first-minute read |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | high rock 재시도도 새 셀 정체로 기각. 1대1 gate만으로 다중 traffic을 보장하지 못함 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | log/wall 계열 엄폐를 다중 traffic gate 뒤 한 개만 맵 검증 |
| T5 자산 승격 | gameplay를 돕는 자산만 런타임으로 승격 | bush와 핵심 icon/audio 일부만 통합 | 자산이 가독성/피드백 문제를 실제로 줄임 | 맵 구조 확정 뒤 landmark 후보 audit |
