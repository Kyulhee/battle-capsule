...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. bot-only v5는 첫 획득 6.3초, 첫 접촉 7.0초, run당 stage1 사망 94-97명이다. hard-bump 첫 획득은 0/5였고 idle reaction/objective interrupt가 첫 교전을 만들었으므로 초기 배치 밀도와 acquisition 경로를 먼저 비교한다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, pacing preset, runtime combat tuning, bot-only simulation participant | observer 분리 5-run 완료. v5 평균 434.7초, spawn 99/99 | 99명 default 승격, release 기준선, v5 기본 승격 | 초기 배치/획득 압력 분리 |
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot-only damage multiplier, nav unstick override | ATTACK 최대 16초, hard-bump first acquisition 0/5. stage1 사망 94-97명 | 완전한 flashlight/fear/battery 전술 | idle/objective acquisition과 spawn density 비교 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | bot-only v5 평균 434.7초, 범위 271.0-655.5초 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | 초기 배치/획득 압력 분리 |
| T2 오프닝 체감 | 첫 1분이 즉사/랜덤 충돌처럼 느껴지지 않게 함 | 첫 접촉 7.0초, stage1 사망 94-97명, hard-bump first acquisition 0/5 | opening 변경이 duration gate를 깨지 않고 수동 플레이에서 납득됨 | spawn/acquisition 후보 뒤 first-minute playtest |
...
1. T1을 먼저 안정화한다. bot-only v5 기준선에서 초기 배치와 first acquisition이 stage1 소모에 미치는 영향을 분리한다.
