...
> 마지막 업데이트: 2026-05-08 (v1.7.1 AI Doctrine Hierarchy Refactor 릴리즈)
> 이 문서는 AI 에이전트 간 인수인계 및 장기 방향 공유를 위해 작성되었습니다.

## 현재 상태

**현재**: v1.7.1 — 다음: v1.7.2 (아키타입 체감 강화 및 시각 안정화)
...
### v1.7.2 — Archetype Readability Stabilization `S`
...
- 남은 작업은 수동 시각 확인과 10회 이상 장기 시뮬레이션으로 편향이 과도하지 않은지 확인하는 것이다.

**헤드리스 검증 기준**
- v1.7.1 대비 5회 평균 duration이 60s 아래로 무너지지 않음.
- zone deaths 0 유지.
...
**현재 미해결 이슈**
- 아키타입이 외형으로는 구분되지만 행동 차이는 아직 약하다. v1.7.2에서 `plan_by_archetype` 기반으로 튜닝한다.
