...
| 전투/AI | bot doctrine, opening guard, player-target 위협 분리, bot damage, nav unstick, 해제 target 수명 방어 | player 표적은 무관한 방관자 때문에 이탈하지 않음. 확장 후보는 이탈 152.4회로 bot-only 전략 실패 노출 | 위협·목표·위치 효용 계층, 완전한 flashlight/fear/battery 전술 | 상태 전이 책임과 효용 입력 분리 |
...
| T2 오프닝 체감 | 첫 1분이 즉사/무접촉 양극단이 아니게 함 | 기존 수동은 무접촉/이탈, 확장 자동은 첫 조우 7.1초 과밀 | 자동 duration과 수동 대인 압박이 함께 납득됨 | AI 효용 결정 계층 |
