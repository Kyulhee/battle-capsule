...
- 최신 map/world slice: N2-MAP-04 route 표현 철회와 전체맵 45도 정렬
...
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot damage, nav unstick, inside-edge zone release | bot-only stage1 과소모와 실제 플레이어 과소압박이 동시에 존재 | 완전한 flashlight/fear/battery 전술 | 대인 target 유지·DISENGAGE 원인 audit |
...
| 맵/경로 | night forest candidate, POI/route telemetry, 45도 정렬 minimap/fullmap | route 표현은 수동 플레이에서 폐기. 맵은 좁고 평지가 비어 보임 | 최종 맵 승격, 자연스러운 병목, 실제 route 이동 계약 | 이동 시간·시야·pickup·장애물 밀도 audit |
...
| T2 오프닝 체감 | 첫 1분이 즉사/무접촉 양극단이 아니게 함 | bot-only 첫 접촉 6.7초지만 실제 플레이어는 정지 시 접촉 부족·교전 이탈 | opening 변경이 duration gate를 깨지 않고 수동 플레이에서 납득됨 | 대인 교전 지속 audit |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 인공 route 표현 폐기, 맵이 좁고 평지가 많다는 수동 신호 | 주요 POI가 물리 동선으로 구분되고 stuck hotspot이 gate 안에 있음 | 맵 규모·밀도 audit |
