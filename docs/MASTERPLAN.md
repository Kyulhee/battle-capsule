...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. v6는 `ZONE_ESCAPE` 수렴과 stuck를 줄였지만 첫 획득 6.2초, 첫 접촉 6.7초, run당 stage1 사망 94-97명으로 attrition이 변하지 않았다. loot 목표 공유 진단은 `idle_loot`의 8.2%에 그쳐 예약 구현을 보류했다. 다음은 acquisition 시간을 늘리지 않고 비전투 이동 중 1-2m 충돌 자체를 줄인다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | v6 spawn 99/99, stuck 0.14. duration 평균 465.1초 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | 초반 비전투 이동 분산 후보 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | v6 평균 465.1초, 범위 236.3-1132.7초 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | 초반 비전투 이동 분산 |
...
1. T1을 먼저 안정화한다. v6 기준선에서 비전투 이동의 근접 충돌이 first acquisition과 stage1 소모에 미치는 영향을 분리한다.
