...
- 최신 gameplay slice: N2-PACE-35 nav 이동 복구와 canonical v4/v5 기준선
- 최신 검증 기반: N2-PACE-34부터 `Main.match_timer`를 canonical time으로 사용하고 navigation bake 완료 뒤 시뮬레이션 시작
- canonical v4 5-run: 평균 426.5초, 범위 246.0-645.6초, first upgrade 233.9초, stage2 230.0초, stage3 600.1초
- nav 수정 v5 5-run: 평균 486.3초, 범위 262.8-664.2초, first upgrade 226.8초, stage2 220.0초, stage3 590.1초, normalized stuck 0.14
- 판단: nav 복구는 구조 수정으로 채택하지만 v4/v5 모두 duration gate를 통과하지 못해 v5는 비기본 후보로 유지

## Night BR 페이싱 판정 창

| 지표 | 목표 창 | 현재 | 판단 |
|---|---|---|---|
| first upgrade | 120-300초 | v5 226.8초 | 통과 |
| stage2 | 240-420초 | v5 220.0초 | 이른 편 |
| stage3 | 540-720초 | v5 590.1초 | 통과 |
| match duration | 600-900초 | v5 평균 486.3초 | 실패 |

첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. 현재 v5는 첫 접촉 7.1초, run당 stage1 사망 93-97명이며 한 run에서 최종 2인 연속 공격 245.5초가 관측됐다. 다음에는 초반 소모와 최종 2인 교착을 분리해 duration 분산의 주 원인을 고른 뒤 수동 플레이로 오프닝과 route/cover 판독을 확인한다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, pacing preset, runtime combat tuning, `run_verify.py` profile | canonical v4/v5 5-run 완료. v5 평균 486.3초 | 99명 default 승격, release 기준선, v5 기본 승격 | 초반/최종 2인 duration 영향 분리 |
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot-only damage multiplier, nav unstick override | nav 수정 뒤 stuck gate 통과. stage1 사망 93-97명, 공격 지속 245.5초 이상치 | 완전한 flashlight/fear/battery 전술 | 상태별 교전 지속과 유효 damage 진단 |
...
| zone/pacing | canonical match clock, nav bake 대기, stage timing, zone pressure telemetry | v4/v5 모두 duration 미달, 같은 seed도 최소 5-run 필요 | 안정된 10-15분 분포 | 조기 종료와 장기 교전 분산 원인 분리 |
| 맵/경로 | night forest candidate, POI/route telemetry, minimap/fullmap 데이터 | nav 이동이 stuck override를 적용해 normalized stuck 0.14 | 최종 맵 승격, hotspot 수동 검증 | obstacle 점 이동 없이 pathing 회귀 감시 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | v4 평균 426.5초, v5 평균 486.3초. v5 범위 262.8-664.2초 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | stage1 소모와 최종 2인 장기 교전 분리 |
| T2 오프닝 체감 | 첫 1분이 즉사/랜덤 충돌처럼 느껴지지 않게 함 | v5 첫 접촉 7.1초, stage1 사망 93-97명, 5초 brush는 실패 | opening 변경이 duration gate를 깨지 않고 수동 플레이에서 납득됨 | T1 원인 분리 뒤 first-minute playtest |
...
| T4 맵/경로 체감 | route choice가 bot collision보다 강하게 느껴짐 | nav unstick 수정으로 gate 통과, hotspot과 수동 체감은 미확정 | 주요 POI/route가 플레이 중 구분되고 stuck hotspot이 gate 안에 있음 | pathing watch + 수동 route read |
...
1. T1을 먼저 안정화한다. canonical v4/v5 기준선에서 stage1 소모와 최종 2인 장기 교전의 영향을 분리한다.
