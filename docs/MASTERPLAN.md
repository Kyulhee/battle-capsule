...
| 맵/경로 | 기존 Night + 260m 확장 whitebox, spawn/목적지·open 피해 위치 계측, 45도 정렬 지도 | open 피해는 기존 장애물 주변에 분산되고 두 high rock 셀에서 정체와 중첩 | 최종 맵 승격, 자연스러운 병목 | Arena high rock ramp·nav 재현 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | open 피해 상위 셀은 기존 바위·나무 주변이며 high rock 두 셀은 정체와 중첩 | 주요 POI가 물리 동선과 AI 목표로 연결되고 stuck gate 통과 | 소형 Arena에서 rock ramp·nav 원인을 분리한 뒤 한 배치만 재설계 |
