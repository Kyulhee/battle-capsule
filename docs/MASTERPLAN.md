...
### v0.7 — HUD/UI 재설계 + 그룹 전술

#### HUD/UI 재설계 (우선 처리)

현재 알려진 UI 문제:
- Zone closing/Zone timer 공지가 HP 상태바와 겹침
- Zone timer label이 중앙 상단에 고정돼 다른 정보(킬피드, 상태바)와 겹치는 구조
- 킬피드 표시 영역과 HUD 영역 간 레이아웃 기준 불명확

목표: HUD 레이아웃을 구역별로 명확히 분리
- **좌상단**: HP · 쉴드 · 힐 아이템 · 생존자 수 (기존 상태바 유지, 위치/크기 정비)
- **중앙 상단**: Zone 타이머 / ZONE CLOSING (기존 위치. 겹침 방지용 배경 패널 추가)
- **우상단**: 킬피드 (현재 위치 유지, 최대 높이 제한 명시)
- **하단 중앙**: 무기 슬롯 바 (변경 없음)

추가 개선:
- Zone closing 시 별도 경고 배너 (화면 상단에 배경 있는 레드 텍스트) 표시
- Kill/Assist 알림을 화면 중앙 아래쪽에 잠깐 크게 띄우는 방식 검토
