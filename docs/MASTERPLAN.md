...
- 최신 map UI slice: N2-MAP-02 minimap/fullmap route 가시화
...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. 미세 AI 예외와 primary route cover 추가 모두 생존 이득보다 pathing 회귀가 컸다. map UI에서는 route 선택이 읽히므로, 다음은 collider 없는 world cue로 같은 동선을 연결한 뒤 수동 첫 1분을 본다.
...
| 맵/경로 | night forest candidate, POI/route telemetry, minimap/fullmap route 표시 | map UI에서 6개 역할 route 판독 가능. world 동선 cue는 없음 | 최종 맵 승격, hotspot 수동 검증, 실제 route 이동 계약 | 비충돌 world route cue |
...
| T3 야간 가독성 | 플레이어/픽업/cover/route가 실제 화면에서 읽힘 | Night world와 map UI 캡처 통과, 실제 장시간 플레이는 미확정 | `visual_review` 캡처와 수동 기록에서 route/cover 판독 가능 | world route cue 뒤 first-minute read |
| T4 맵/경로 체감 | route choice가 bot collision보다 강하게 느껴짐 | map UI route는 판독 가능, world에는 같은 cue가 없음 | 주요 POI/route가 플레이 중 구분되고 stuck hotspot이 gate 안에 있음 | 비충돌 world route cue |
