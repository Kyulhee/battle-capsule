...
첫 1분은 즉사나 랜덤 충돌보다 위험을 읽고 선택하는 느낌이어야 한다. guard 연장, `ZONE_ESCAPE` 복귀 분리, loot 공유 진단, 12초 비전투 근접 분산은 stage1 사망 95명대를 바꾸지 못했고, stage1 피해 0.35도 92.4명까지밖에 줄이지 못한 채 `DISENGAGE` 고착을 키웠다. 다음은 피해량을 더 낮추지 않고 사망 직후 즉시 표적을 갈아타는 post-kill 연쇄만 짧게 끊는다.

## 현재 구현 상황

| 영역 | 구현됨 | 후보/검증 신호 | 아직 아님 | 다음 판단 |
|---|---|---|---|---|
| 매치/preset | map spec 기반 match 구성, runtime combat/bot tuning, bot-only simulation participant | v6 spawn 99/99, stuck 0.14. stage1 피해 v7은 gate 실패 | 99명 default 승격, release 기준선, v5/v6 기본 승격 | post-kill 재획득 후보 |
| 전투/AI | bot doctrine, opening guard, 4초 hard-bump brush, bot damage, nav unstick, inside-edge zone release | 피해 감소는 사망 -3.2명보다 DISENGAGE stuck 회귀가 큼 | 완전한 flashlight/fear/battery 전술 | stage1 post-kill 즉시 재획득 제한 |
...
| zone/pacing | canonical match clock, nav bake 대기, bot-only participant, stage timing | v6 465.1초, 피해 v7 551.3초지만 stuck/upgrade gate 실패 | 안정된 10-15분 분포 | attrition 연쇄 완화 후보 |
...
|---|---|---|---|---|
| T1 페이싱 안정화 | 안정된 10-15분 match 분포 확보 | v6 stage1 사망 95.6명, 피해 0.35 v7도 92.4명과 stuck 0.19 | canonical 5-run 이상에서 평균 600-900초와 개별 run 허용 범위를 함께 만족, timing band 유지 | stage1 post-kill 재획득 연쇄 제한 |
...
1. T1을 먼저 안정화한다. v6의 피해량과 stage2 이후 전투는 유지하고, stage1 사망 직후 여러 공격자가 즉시 다음 표적을 잡는 연쇄만 제한한다.
