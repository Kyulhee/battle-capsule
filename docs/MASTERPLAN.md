...
- 최신 asset slice: N2-ASSET-01 무기 4종·지면 발걸음 3종 런타임 승격, catalog 누락 0
...
- N2-ASSET-01: catalog에만 있던 무기 4종·지면 발걸음 3종 WAV를 런타임으로 승격하고 raw WAV fallback과 ID별 캐시를 추가했다. 누락 0 smoke와 `unit_smoke`를 통과했고 60봇 p95 15.5-16.5ms를 유지했다
...
| 자산/audio | 핵심 icon, 무기 4종·지면 발걸음 3종 WAV, bush GLB 런타임 통합 | catalog 누락 0, 스트림 캐시, 60봇 p95 15.5-16.5ms | 오디오 수동 청취, rock/tree/landmark 대량 승격 | 거점 동선과 오디오를 함께 수동 판정 |
...
| T4 맵/경로 체감 | pickup과 지형이 자연스러운 이동·병목 선택을 만듦 | 중앙·남쪽 엄폐 후보가 자동 구조·경로·성능 gate 통과 | 주요 POI가 물리 동선과 AI 목표로 연결되고 수동 체감 통과 | N2-PLAY-03 중앙·남쪽 접근 비교 |
| T5 자산 승격 | gameplay를 돕는 자산만 런타임으로 승격 | 핵심 오디오 7종 누락 해소, bush와 핵심 icon 통합 | 자산이 가독성/피드백 문제를 실제로 줄임 | N2-PLAY-03 청취 뒤 landmark 시각·충돌 계약 audit |
