"""E080/E081 수집 시계 관측과 선택적 전체 매치 pair. 기본값은 짧은 창이며 승격하지 않는다."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    parser.add_argument("--clock-candidate", action="store_true", help="Compare E081 physics clock instead of E078 patrol.")
    parser.add_argument("--full-match", action="store_true", help="One OFF/ON E081 integration pair; no promotion claim.")
    ...
    if args.full_match and (not args.clock_candidate or args.start_delay_ms):
        parser.error("--full-match requires --clock-candidate and excludes delay injection.")
    ...
    save(output / "inputs.json", dict(reference=str(args.reference_flow.resolve()), reference_sha256=digest(args.reference_flow),
         ...
         candidate="physics_clock" if args.clock_candidate else "recovery_patrol",
    ...
    for name, flags in cases:
        ...
        if not data["complete"] or result.exists() != args.full_match or data["snapshots"][0] != initial:
