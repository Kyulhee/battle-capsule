...
const PROBE_PATH := "res://data/mapSpec_poi_sluice_crossing_probe.json"
...
func _init():
	...
	if not definition.has_scale_preset(PRESET):
		_fail("Sluice probe is missing scale preset '%s'." % PRESET)
		return

	var issues: Array = definition.validate(game_config, PRESET)
	if not issues.is_empty():
		_fail("Sluice probe validation failed: %s" % _join_issues(issues))
		return
	...
	print("Sluice crossing POI probe smoke passed: pois=%d routes=%d roles=%s route_roles=%s." % [
		pois.size(),
		routes.size(),
...
func _verify_poi_roles(pois: Array[Dictionary]) -> bool:
	...
	for role in REQUIRED_POI_ROLES.keys():
		...
		if actual < required:
			_fail("Sluice probe needs at least %d POIs with role '%s'; got %d." % [required, role, actual])
			return false
	return true
...
func _verify_routes(routes: Array[Dictionary], pois: Array[Dictionary]) -> bool:
	...
	for role in REQUIRED_ROUTE_ROLES.keys():
		...
		if actual < required:
			_fail("Sluice probe needs at least %d routes with role '%s'; got %d." % [required, role, actual])
			return false
	...
	for route in routes:
		...
		if width < 6.0:
			_fail("Route %s width is too narrow for POI probing: %.1f." % [route.get("id", ""), width])
			return false

		var route_id := String(route.get("id", ""))
		var role := String(route.get("role", ""))
		if route_id == "sluice_direct_crossing":
			direct_route_found = true
			if role != "primary_choke":
				_fail("sluice_direct_crossing must be primary_choke, got %s." % role)
				return false
			if points.size() < 5:
				_fail("sluice_direct_crossing must be a multi-point route.")
				return false
			if width > 10.5:
				_fail("sluice_direct_crossing is too wide for exposed-crossing pressure: %.1f." % width)
				return false
			...
			if alternate_id.is_empty() or not route_ids.has(alternate_id):
				_fail("sluice_direct_crossing needs a valid alternate_route_id.")
				return false
	...
	if not direct_route_found:
		_fail("Sluice probe is missing sluice_direct_crossing.")
		return false
	return true


func _verify_position_classification(definition) -> bool:
	var crossing: Dictionary = definition.describe_strategic_position(Vector2.ZERO)
	if String(crossing.get("poi_name", "")) != "Sluice Crossing":
		_fail("Center should classify as Sluice Crossing, got %s." % crossing.get("poi_name", ""))
		return false
	if String(crossing.get("route_id", "")) != "sluice_direct_crossing":
		_fail("Center should classify on sluice_direct_crossing, got %s." % crossing.get("route_id", ""))
		return false
	if String(crossing.get("route_role", "")) != "primary_choke":
		_fail("Center should classify as primary_choke, got %s." % crossing.get("route_role", ""))
		return false

	var north: Dictionary = definition.describe_strategic_position(Vector2(-12.0, 24.0))
	if String(north.get("poi_name", "")) != "North Pines":
		_fail("North flank should classify as North Pines, got %s." % north.get("poi_name", ""))
		return false
	if String(north.get("route_role", "")) != "flank":
		_fail("North flank should classify as flank, got %s." % north.get("route_role", ""))
		return false

	var south: Dictionary = definition.describe_strategic_position(Vector2(20.0, -10.0))
	if String(south.get("poi_name", "")) != "South Reeds":
		_fail("South flank should classify as South Reeds, got %s." % south.get("poi_name", ""))
		return false
	if String(south.get("route_role", "")) != "flank":
		_fail("South flank should classify as flank, got %s." % south.get("route_role", ""))
		return false

	var pump: Dictionary = definition.describe_strategic_position(Vector2(8.0, -24.0))
	if String(pump.get("poi_name", "")) != "Pump Shelter":
		_fail("Pump point should classify as Pump Shelter, got %s." % pump.get("poi_name", ""))
		return false
	if String(pump.get("poi_role", "")) != "recovery_pocket":
		_fail("Pump point should classify as recovery_pocket, got %s." % pump.get("poi_role", ""))
		return false
	if String(pump.get("route_role", "")) != "flank":
		_fail("Pump point should remain on the south flank route before recovery exit, got %s." % pump.get("route_role", ""))
		return false

	var reentry: Dictionary = definition.describe_strategic_position(Vector2(12.0, -14.0))
	if String(reentry.get("route_role", "")) != "recovery_exit":
		_fail("Pump reentry segment should classify as recovery_exit, got %s." % reentry.get("route_role", ""))
		return false
	return true
