...
const OUTPUT_PATH := "C:/tmp/full_map_routes.png"
const MINIMAP_OUTPUT_PATH := "C:/tmp/minimap_routes.png"
...
func _capture() -> void:
	...
	if file == null or not definition.load_from_json(file.get_as_text(), MAP_PATH, game_config):
		push_error("Could not load the Night map for route capture.")
		quit(1)
		return
	...
	if image == null:
		push_error("Could not capture full-map routes.")
		quit(1)
		return
	var err := image.save_png(OUTPUT_PATH)
	if err != OK:
		push_error("Could not save full-map route capture.")
		quit(1)
		return
	print("Full-map route capture saved: %s" % OUTPUT_PATH)
	...
	if minimap_image == null or minimap_image.save_png(MINIMAP_OUTPUT_PATH) != OK:
		push_error("Could not save minimap route capture.")
		quit(1)
		return
	print("Minimap route capture saved: %s" % MINIMAP_OUTPUT_PATH)
	quit(0)
